...
an address to the `Symbol` covering it. A failure carries its Rust trace on
...
`cc=CallingConvention.arm_aapcs_soft()` or its float arguments read as empty
registers.
...
`LifterOptions(assumptions=AssumptionOptions(...))`; `alias_mode` is
`"stack_global_disjoint"` by default and `"strict"` is the always-sound floor.
