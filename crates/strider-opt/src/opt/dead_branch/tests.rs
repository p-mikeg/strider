...
/// `n` constant gates, each `If(true)` handing its live arm to an inner
/// `If(true)` whose own live arm joins one shared exit-free spin and whose dead
/// arm is the only route on toward the `Return`.  No gate is in the escape set
/// (every route out crosses a dead arm), so each falls through to the exact
/// walk.
fn gate_over_inner_gate_chain(n: usize) -> Result<strider_ir::Function> {
    let mut b = strider_ir_test_utils::empty_builder()?;
    let entry = b.create_region_all()?;
    let gates: Vec<_> = (0..n)
        .map(|_| b.create_region_all())
        .collect::<Result<Vec<_>>>()?;
    let inners: Vec<_> = (0..n)
        .map(|_| b.create_region_all())
        .collect::<Result<Vec<_>>>()?;
    let spin_head = b.create_region_all()?;
    let spin_tail = b.create_region_all()?;
    ...
    for i in 0..n {
        b.set_region(gates[i]);
        let cond = b.build_boolean_const(true);
        b.build_if(cond, inners[i], spin_head)?;

        b.set_region(inners[i]);
        let cond = b.build_boolean_const(true);
        let onward = if i + 1 < n { gates[i + 1] } else { exit };
        b.build_if(cond, spin_head, onward)?;
    }
...
/// Positive verdicts must carry across roots: without a shared one, each gate
/// pays its own whole-CFG walk.
#[test]
fn a_chain_of_gates_over_inner_gates_walks_the_cfg_once() -> Result<()> {
    for n in [8usize, 64] {
        let mut fg = gate_over_inner_gate_chain(n)?;
        super::FULL_WALKS.with(|c| c.set(0));
        crate::pipeline::run_one(&DeadBranchElimination, &mut fg, &mut OptCtx::new(None))?;
        let walks = super::FULL_WALKS.with(std::cell::Cell::get);
        assert!(
            walks <= 2,
            "{n} gates over one chain must share their walks, ran {walks}"
        );
    }
    Ok(())
}
