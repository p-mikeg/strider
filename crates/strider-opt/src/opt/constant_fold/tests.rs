...
// ── power-of-two strength reduction (mul/udiv → shift) ────────────────────

#[test]
fn mul_by_pow2_lowers_to_shl() -> Result<()> {
    // x * 8 → x << 3
    let vn = reg_vn(0x1000, 8);
    let (mut fg, x) = make_fn_with_var(vn, |b, x| {
        let c8 = b.build_int_const(8u64, ValueType::I64).unwrap();
        b.build_int_binary_operation(x, c8, IntBinaryOp::Mul, ValueType::I64)
    })?;
    run_to_fixed_point(&ConstantFold::new(), &mut fg)?;
    let node = fg.producer(return_value(fg.graph())?);
    assert!(
        matches!(fg.node_kind(node), NodeKind::IntBinaryOp(IntBinaryOp::ShiftLeft)),
        "x*8 must become a left shift, got {:?}",
        fg.node_kind(node)
    );
    let inputs: Vec<_> = fg.node_inputs(node).into_iter().collect();
    assert!(inputs.contains(&x), "the shifted value must be x");
    assert!(
        inputs.iter().any(|&o| fg.int_const_u128(o) == Some(3)),
...
#[test]
fn mul_by_non_pow2_stays_mul() -> Result<()> {
    // x * 6 stays a Mul (only pow2 is strength-reduced).
    let vn = reg_vn(0x1000, 8);
    let (mut fg, _x) = make_fn_with_var(vn, |b, x| {
        let c6 = b.build_int_const(6u64, ValueType::I64).unwrap();
        b.build_int_binary_operation(x, c6, IntBinaryOp::Mul, ValueType::I64)
    })?;
    run_to_fixed_point(&ConstantFold::new(), &mut fg)?;
    let node = fg.producer(return_value(fg.graph())?);
    assert!(matches!(
...
#[test]
fn udiv_by_pow2_lowers_to_shr() -> Result<()> {
    // x / 8 (unsigned) → x >> 3 (logical)
    let vn = reg_vn(0x1000, 8);
    let (mut fg, x) = make_fn_with_var(vn, |b, x| {
        let c8 = b.build_int_const(8u64, ValueType::I64).unwrap();
        b.build_int_binary_operation(x, c8, IntBinaryOp::Div, ValueType::I64)
    })?;
    run_to_fixed_point(&ConstantFold::new(), &mut fg)?;
    let node = fg.producer(return_value(fg.graph())?);
    assert!(
        matches!(fg.node_kind(node), NodeKind::IntBinaryOp(IntBinaryOp::ShiftRight)),
        "x/8 must become a logical right shift, got {:?}",
        fg.node_kind(node)
    );
    assert!(fg.node_inputs(node).into_iter().collect::<Vec<_>>().contains(&x));
    Ok(())
}

#[test]
fn sdiv_by_pow2_stays_sdiv() -> Result<()> {
    // Signed x / 8 must NOT become a shift (rounding toward zero differs).
    let vn = reg_vn(0x1000, 8);
    let (mut fg, _x) = make_fn_with_var(vn, |b, x| {
        let c8 = b.build_int_const(8u64, ValueType::I64).unwrap();
        b.build_int_binary_operation(x, c8, IntBinaryOp::Sdiv, ValueType::I64)
    })?;
    run_to_fixed_point(&ConstantFold::new(), &mut fg)?;
    let node = fg.producer(return_value(fg.graph())?);
    assert!(
        matches!(fg.node_kind(node), NodeKind::IntBinaryOp(IntBinaryOp::Sdiv)),
        "signed div by pow2 must stay Sdiv, got {:?}",
        fg.node_kind(node)
    );
    Ok(())
}
