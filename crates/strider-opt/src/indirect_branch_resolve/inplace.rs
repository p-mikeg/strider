...
/// the orchestrator via [`crate::AnchorCallingContext`] and wired
...
pub fn apply_tail_call(
    ...
    let mut call_outputs: Vec<ValueKind> = Vec::with_capacity(2 + clobbered_kinds.len());
...
mod tests {
    ...
    fn synth_value_output(
        ...
        let nid =
            function
                .graph_mut()
                .create_node(NodeKind::IntConst(value), [], [ValueKind::Typed(ty)]);
