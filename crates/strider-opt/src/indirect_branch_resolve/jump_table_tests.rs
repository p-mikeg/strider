...
fn bound_via_known_bits_returns_max_plus_one() {
    ...
    let known = analyze_known_bits(&g)
        .expect("kb analyze");
...
fn bound_via_known_bits_handles_zero_extend() {
    ...
    builder
        .build_indirect_branch(placeholder)
        .expect("build_indirect_branch");
    builder.set_lift_addr(None);
...
fn build_pred_if_graph(bound: u64) -> (Function, ValueId, ValueId) {
    ...
    (
        function,
        anchor.expect("placeholder return"),
        idx_in_dispatch,
    )
}
...
fn build_diamond_two_bounds(bound_a: u64, bound_b: u64) -> (Function, ValueId, ValueId) {
    use strider_ir::IntCmpOp;
    let idx_var = rsleigh::Vn {
        addr_off: 0x10,
        addr_space: rsleigh::VnSpace::REGISTER,
