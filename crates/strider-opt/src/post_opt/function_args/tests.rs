...
/// A `CallOther` reaches the memory chain only when its ABI says it clobbers
/// memory, and once there it blocks under either setting of
/// `assume_incoming_args_survive_calls`: the callee is opaque, so nothing can
/// be inferred from its arguments.
#[test]
fn call_other_on_the_chain_blocks_under_either_survival_setting() -> Result<()> {
    let sp = sp32_vn();
    ...
    assert!(
        ...
        "default (incoming args survive calls): a CallOther on the chain still \
...
/// arguments, not what the memory edges say. Narrowing a load's memory input
/// past a `Call` outlives the pass, so a later pipeline run with the relaxation
/// off would inherit an answer it never opted into.
#[test]
...
/// A user-op whose ABI declares it clobbers memory blocks the chain whatever
/// `assume_incoming_args_survive_calls` says. That relaxation speaks for a
/// conforming callee's frame discipline; a syscall handed a pointer into the
/// incoming-argument area has none.
#[test]
fn memory_clobbering_call_other_blocks_regardless_of_the_survival_flag() -> Result<()> {
    ...
    assert!(
        fg.side_tables().arg_index_to_values(0).is_empty(),
        "a memory-clobbering user-op must dirty the chain"
    );
    Ok(())
}
