...
fn reassoc_no_fold_without_const() -> Result<()> {
    ...
    let res = ConstantFold::new().optimize(&mut fg, &crate::OptCtx::empty())?;
...
fn fold_bool_xor_true_to_not() -> Result<()> {
    ...
    let _ = ConstantFold::new().optimize(&mut fg, &crate::OptCtx::empty())?;
    assert_eq!(
...
fn build_unary_with_wide_const_input(
    ...
    let wide_node =
        fg.graph_mut()
            .create_node(NodeKind::IntConst(0xFF), [], [ValueKind::Typed(wide_ty)]);
...
fn fold_lzcount_u128_input_skips_cleanly() -> Result<()> {
    ...
    let result = ConstantFold::new().optimize(&mut fg, &crate::OptCtx::empty());
    assert!(
        result.is_ok(),
...
fn single_pass_propagates_through_chain() -> Result<()> {
    ...
    ConstantFold::new().optimize(&mut fg, &crate::OptCtx::empty())?;
