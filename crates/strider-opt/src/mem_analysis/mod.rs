...
fn alignment_masked_operand(function: &Function, node: NodeId) -> Option<ValueId> {
    if !matches!(
...
fn offset_range_verdict(load: SizedAddr, store: SizedAddr) -> AliasVerdict {
    ...
    if load_off == store_off {
        AliasVerdict::Match
    } else if !offsets_comparable(load, store, load_off, store_off) {
...
fn offsets_comparable(load: SizedAddr, store: SizedAddr, load_off: i128, store_off: i128) -> bool {
    let Some(bits) = load.addr_bits.or(store.addr_bits) else {
        return true;
    };
    if bits >= 128 {
        return true;
...
pub(crate) fn alias_verdict(
    ...
    match (load.class, store.class) {
        ...
        // Heap allocations are sound-disjoint (noalias), not heuristic: two
        // distinct allocation bases never overlap, and no allocation coincides
        // with the stack or a global.  Same base falls through to offsets;
...
impl MemorySSAWalker for MemWalker<'_> {
    fn def_clobbers(&mut self, function: &Function, def: NodeId) -> bool {
        ...
        match *function.node_kind(def) {
            ...
            // `preserves_memory` is a per-user-op ABI attribute, read the same
            // way as a `Call`'s.  No frame relaxation applies: an opaque
            // user-op may write the stack without a frame-address argument.
            NodeKind::CallOther { .. } => {
                !function.get_cc(def).preserves_memory && self.analyzer.options.calls_block
            }
...
impl MemWalker<'_> {
    ...
    fn private_frame_forward(&self, function: &Function) -> bool {
        let AddrClass::StackRooted { base, offset } = self.load.class else {
            return false;
        };
        self.analyzer.options.call_relaxations
            && self.analyzer.options.escape_analysis
            && in_own_frame(function, base, offset)
...
/// Does `base + offset` name THIS function's own frame?
///
...
/// PPC64 ELFv1 48 / ELFv2 32.  An ABI declaring none bounds at 0, since the
/// caller's frame still starts at the entry SP.
///
/// Every stack base is at or below the entry SP: [`decompose`] roots one either
/// at `InitialVar(sp)` or at an alignment mask of it, and masking only clears
/// bits.  So the same comparison holds in either base's coordinates.
fn in_own_frame(function: &Function, base: ValueId, offset: i128) -> bool {
    ...
    let rooted_at_entry_sp = match *function.node_kind(base_node) {
        ...
        _ => alignment_masked_operand(function, base_node).is_some(),
    };
    rooted_at_entry_sp
        && offset
            < function
...
impl MemAnalyzer {
    ...
    fn arg_window_covers(
        ...
        let window = scan_arg_window(function, self.options, geometry, hi);
