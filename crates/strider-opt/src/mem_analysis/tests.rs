...
mod ranges_tests {
    ...
    fn ranges_disjoint_max_size_left_does_not_panic_and_is_conservative() {
        // With plain `+`, `a_off + i128::MAX` panics in debug and wraps in
        // release for any positive `a_off`.  Real SP-relative offsets are small,
        // so cover zero and modest magnitudes of both signs.
...
mod cfg_tests {
    ...
    /// `memory_offsets` still records it as `[sp+K]`, and `verdict` must
    ...
    fn verdict_uses_stack_offset_ssot_for_nondecomposable_store() {
        ...
        let cfg = MemAnalyzer::new(MemOptions::call_blocking(AliasMode::StackGlobalDisjoint));
        assert_eq!(
            cfg.verdict(&f, load, store),
            AliasVerdict::Match,
            "verdict must classify the store via the memory_offsets SSoT (like def_clobbers), \
...
mod heap_tests {
    ...
    fn two_heap_objects_are_disjoint() -> crate::Result<()> {
        ...
        let load = fg
            ...
            .expect("load node");
        let cfg = MemAnalyzer::new(MemOptions::call_blocking(AliasMode::Strict));
        assert_eq!(
    ...
    fn allocator_does_not_forward_its_own_stack_argument_slot() -> crate::Result<()> {
        ...
        let mem = fg.node_inputs(load)[0];
        let analyzer = MemAnalyzer::new(MemOptions::call_blocking(
    ...
    fn incoming_stack_arg_does_not_forward_across_a_call() -> crate::Result<()> {
        ...
        let analyzer = MemAnalyzer::new(
            MemOptions::call_blocking(crate::AliasMode::StackGlobalDisjoint)
...
mod own_frame_tests {
    ...
    fn only_an_alignment_masked_sp_is_a_frame_base() -> crate::Result<()> {
        ...
        assert!(
            in_own_frame(&fg, aligned, -8),
            "an alignment-masked SP is this frame's base"
        );
        assert!(
            !in_own_frame(&fg, extracted, -8),
...
mod modular_offset_tests {
    ...
    fn offsets_two_bytes_apart_across_the_wrap_may_alias() -> crate::Result<()> {
        ...
        let store_node = fg
            ...
            .find(|&n| matches!(fg.node_kind(n), NodeKind::Store(_)))
            .expect("store");
        let analyzer = MemAnalyzer::new(MemOptions::call_blocking(crate::AliasMode::Strict));
        assert_ne!(
