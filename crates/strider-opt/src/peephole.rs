...
pub(crate) trait PeepholePass {
    ...
    /// `root` always matches [`Self::matches_kind`]; no defensive re-check
    /// is needed.
    ///
    /// # Errors
...
pub(crate) fn run_peephole<P: PeepholePass>(
    ...
    while let Some(root) = work.dequeue() {
        ...
        if let PeepholeRewrite::Changed { new_node } = r {
            ...
            // A rewrite may build a fresh node (a folded constant, a merged
            // AND-mask) that the same pass can immediately rewrite again.
            // Without re-enqueueing it, whether that fires within one sweep
