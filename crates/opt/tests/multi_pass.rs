...
fn dbe_strips_phi_then_redundant_phis_collapses() -> opt::Result<()> {
    ...
    b.set_region(true_r);
    let v_t = b.build_int_const(11u64, NodeOutputType::U64).unwrap();
    b.write_variable(&var, v_t)?;
    b.build_branch(join)?;
    b.set_region(false_r);
    let v_f = b.build_int_const(22u64, NodeOutputType::U64).unwrap();
...
fn nested_const_branches_fully_eliminated() -> opt::Result<()> {
    ...
    let dead_v = b.build_int_const(99u64, NodeOutputType::U64).unwrap();
...
fn default_pipeline_exercises_all_passes() -> opt::Result<()> {
    ...
    let c2 = b.build_int_const(4u64, NodeOutputType::U8).unwrap();
    ...
    b.set_region(live);
    let v = b.build_int_const(42u64, NodeOutputType::U64).unwrap();
...
fn pipeline_no_change_on_already_optimal() -> opt::Result<()> {
    let mut fg = make_fn(|b| Ok(b.build_int_const(7u64, NodeOutputType::U64).unwrap()))?;
    default_pipeline().run(&mut fg.graph, fg.entry)?;
