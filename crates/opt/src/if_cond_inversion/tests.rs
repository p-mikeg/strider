...
fn build_if_with_neg_cond() -> Result<(ir::BuiltFunctionGraph, ir::node::NodeId)> {
    let cond_vn = ir::test_utils::reg_vn(0x1000, 1);
...
fn if_cond_kind(fg: &ir::BuiltFunctionGraph, if_node: ir::node::NodeId) -> NodeKind {
...
fn double_neg_collapses_after_constant_fold() -> Result<()> {
    ...
    let n2 = b.build_boolean_unary_operation(n1, ir::BoolUnaryOp::Neg)?;
    b.build_if(n2, t, f)?;
    b.set_region(t);
    let one = b.build_int_const(1u64, NodeOutputType::U64)?;
    b.build_return(Some(one), &[])?;
...
fn swap_consumers_preserves_value_semantics() -> Result<()> {
    ...
    let (mut fg, if_node) = build_if_with_neg_cond()?;
    let consumer_of = |fg: &ir::BuiltFunctionGraph, out: ir::node::NodeOutputId| -> ir::node::NodeId {
