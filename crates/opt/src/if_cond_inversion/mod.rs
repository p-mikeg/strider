...
fn is_inverted_cond(graph: &ir::Graph, if_node: NodeId) -> bool {
...
fn invert(graph: &mut ir::Graph, if_node: NodeId) -> Result<()> {
    ...
    let true_use_ids: smallvec::SmallVec<[ir::node::NodeInputId; 4]> = graph
        .output_uses(true_out)
        .map(|(consumer, idx)| graph.node_input_id_at(consumer, idx as usize))
        .collect::<Result<_>>()?;
    let false_use_ids: smallvec::SmallVec<[ir::node::NodeInputId; 4]> = graph
