...
// ── F1 fingerprint propagation through known_bits folds ─────────────────────

#[test]
fn known_bits_fold_unions_input_fingerprints() -> Result<()> {
    // KnownBits collapses `(x | 7) & 4` to `IntConst(4)` when bits 0-2 of
    // the Or are statically 1 and the And mask is 4.  The replacement
    // IntConst must inherit the root's fingerprint (auto-merged + the
    // root's seeded addr).  Pin: every operand addr survives the fold.
    let addr_x = ir::PcodeInsnAddr::new(0x1000, 0);
    let addr_7 = ir::PcodeInsnAddr::new(0x1004, 0);
    let addr_4 = ir::PcodeInsnAddr::new(0x1008, 0);
    let mut fg = make_fn(|b| {
        let x_seed = b.build_int_const(0u64, NodeOutputType::U64);
        let c7 = b.build_int_const(7u64, NodeOutputType::U64);
        let c4 = b.build_int_const(4u64, NodeOutputType::U64);
        // Seed BEFORE building any combining node so create_node's
        // auto-merge picks up the seeded addrs at construction time.
        let x_node = b.body().graph.get_node_from_output(x_seed);
        let c7_node = b.body().graph.get_node_from_output(c7);
        let c4_node = b.body().graph.get_node_from_output(c4);
        b.body_mut()
            .graph
            .set_fingerprint(x_node, ir::Fingerprint::from_single(addr_x));
        b.body_mut()
            .graph
            .set_fingerprint(c7_node, ir::Fingerprint::from_single(addr_7));
        b.body_mut()
            .graph
            .set_fingerprint(c4_node, ir::Fingerprint::from_single(addr_4));
        let ored = b.build_int_binary_operation(x_seed, c7, IntBinaryOp::Or, NodeOutputType::U64)?;
        Ok(b.build_int_binary_operation(ored, c4, IntBinaryOp::And, NodeOutputType::U64)?)
    })?;
    let mut changed = true;
    ...
    assert_eq!(fg.graph.node_kind(producer), &NodeKind::IntConst(4));
    let fp = fg.graph.fingerprint_of(producer);
    assert!(
        fp.contains(addr_x) && fp.contains(addr_7) && fp.contains(addr_4),
        "folded IntConst(4) must carry every operand's seeded addr — \
         provenance must survive known-bits folding; got {:?}",
