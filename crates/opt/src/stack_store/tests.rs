...
fn phi_sp_collapses_to_stack_store() -> Result<()> {
    ...
    b.set_region(body);
    let sp_val = b.read_variable(&sp)?;
    let data = b.build_int_const(0xAB, NodeOutputType::U32).unwrap();
...
fn phi_of_offsets_becomes_stack_store_phi() -> Result<()> {
    ...
    let data = b.build_int_const(0xCC, NodeOutputType::U32).unwrap();
...
fn phi_with_equal_offsets_collapses_to_stack_store() -> Result<()> {
    ...
    b.set_region(bb);
    let sp_b = b.read_variable(&sp)?;
    let four2 = b.build_int_const(4, NodeOutputType::U32).unwrap();
    let sp_b2 =
...
fn buf_init_does_not_leak_into_args() -> Result<()> {
    ...
    let four = b.build_int_const(4, NodeOutputType::U32).unwrap();
    let sixteen = b.build_int_const(16, NodeOutputType::U32).unwrap();
    ...
    let init_ebx = b.build_int_const(0xEB, NodeOutputType::U32).unwrap();
    ...
    let arg1 = b.build_int_const(1, NodeOutputType::U32).unwrap();
    ...
    let arg0 = b.build_int_const(42, NodeOutputType::U32).unwrap();
    ...
    let retaddr = b.build_int_const(0x1234, NodeOutputType::U32).unwrap();
    b.build_store(sp_call, retaddr, rsleigh::VnSpace::RAM)?;

    // call target.
    let target = b.build_int_const(0x1000, NodeOutputType::U32).unwrap();
    b.build_call(target)?;
    b.build_return(None, &[])?;
    let mut fg = b.build()?;
...
fn non_stack_store_is_untouched() -> Result<()> {
    ...
    b.set_entry_region(region)?;
    b.set_region(region);
    let addr = b.build_int_const(0x1000, NodeOutputType::U32).unwrap();
    let data = b.build_int_const(0x42, NodeOutputType::U32).unwrap();
    b.build_store(addr, data, rsleigh::VnSpace::RAM)?;
    b.build_return(None, &[])?;
    let mut fg = b.build()?;
...
fn cdecl_two_stack_args_collected_in_order() -> Result<()> {
    ...
    let arg1 = b.build_int_const(22, NodeOutputType::U32).unwrap();
    ...
    let arg0 = b.build_int_const(11, NodeOutputType::U32).unwrap();
...
fn missing_slot_zero_skips_collection() -> Result<()> {
    ...
    let sp_v1 =
        b.build_int_binary_operation(sp_v0, four, IntBinaryOp::Sub, NodeOutputType::U32)?;
    b.write_variable(&sp, sp_v1)?;
    let only_arg = b.build_int_const(99, NodeOutputType::U32).unwrap();
...
fn detect_mixed_add_sub_reduces() -> Result<()> {
    ...
    let s16 = b.build_int_const(16, NodeOutputType::U32).unwrap();
    let s4 = b.build_int_const(4, NodeOutputType::U32).unwrap();
    let plus16 =
