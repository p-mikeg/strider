...
pub fn apply_elf_relocations(
    ...
    let owners = super::sections::loaded_section_indices(obj, loaded_with)?;
    ...
    for_each_reloc_site(obj, &owners, &layout, |site_addr, reloc| {
...
fn apply_one_relocation(
    ...
    let covering = |size_bytes| region_index.covering_index(regions, site_addr, size_bytes);
    ...
    // Their value is plain S: an `SHT_REL` slot's field holds the PLT push
    // offset, never an addend.
    ...
    if let Some(size_bytes) = mips_rel32_symbol_reloc_size(reloc, obj.architecture(), endian_le) {
        ...
        let site_region = covering(size_bytes);
...
impl RegionStartIndex {
    ...
    fn covering_index(
        ...
        let site_end = site_addr.checked_add(size_bytes as u64)?;
...
/// Records the low `size_bytes` of `value` at `site_addr`, against `covering`.
///
/// A `None` `covering` silently skips: either the site is unmapped, or its
...
fn record_patch(
    ...
    if let Some(i) = covering
        && let Some(patch) = Patch::new(site_addr, value, size_bytes, endian_le)
    {
...
mod tests {
    ...
    fn covering_index_falls_through_to_a_fully_covering_lower_start_region() {
        ...
        assert_eq!(index.covering_index(&regions, 0x1080, 4), Some(1));
        assert_eq!(index.covering_index(&regions, 0x1080, 8), Some(0));
        assert_eq!(index.covering_index(&regions, 0x10fc, 8), None);
        assert_eq!(index.covering_index(&regions, 0x9000, 1), None);
    }
}
