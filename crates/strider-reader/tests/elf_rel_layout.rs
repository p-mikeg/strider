...
use common::elf_fixture::{
    SectionSpec, SymbolSpec, build_elf_with_sections, build_elf_with_sections_and_symbols,
};
...
fn all_loader_digests(owned: &strider_reader::OwnedElf) -> Vec<u64> {
    ...
    vec![
        ...
        digest(&regions(Auto, ImmutableOnly, false)),
        digest(&regions(Auto, AllAllocatable, false)),
...
fn a_tls_section_is_not_loaded_and_never_shadows_code() {
    let bytes = build_elf_with_sections(&[
        SectionSpec::tdata(0, vec![0xaa; 8]),
        SectionSpec::text(0, vec![0x90; 0x20]),
    ]);
    let obj = object::File::parse(&bytes[..]).unwrap();
    let layout = ElfSectionLayout::new(&obj);
    let text = obj.section_by_name(".text").unwrap();
    let base = layout.section_base(&text);

    for filter in [
        elf::LoadFilter::CodeAndReadOnly,
        elf::LoadFilter::ImmutableOnly,
        elf::LoadFilter::AllAllocatable,
    ] {
        let regions = common::regions(&bytes, filter);
        assert!(
            regions
                .iter()
                .all(|r| common::region_bytes(r) != vec![0xaa; 8]),
            ".tdata must not be loaded"
        );
        if matches!(filter, elf::LoadFilter::CodeAndReadOnly) {
            let table = MemRegionsLookupTable::new(regions);
            let mut got = vec![0u8; 0x20];
            table.read_exact(base, &mut got).unwrap();
            assert_eq!(got, vec![0x90; 0x20], ".text at {base:#x} serves .tdata");
        }
    }
}
...
/// SHT_NOBITS has no file bytes, so its `sh_size` is never validated. Two
