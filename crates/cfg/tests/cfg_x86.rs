//! CFG integration tests for 32-bit x86 (`binary_tests/binary_test`).
//!
//! Tests verify structural properties of the CFG built from real compiled code:
//! region counts, edge kinds, cycle detection, and invariants.
...
fn binary() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/../../binary_tests/binary_test")
}
...
// ── linear functions (single region, no branches) ─────────────────────────────
...
// ── nested-call function (tail call optimised) ────────────────────────────────

/// `g` tail-calls into `add`; with default options the branch is treated as a
/// tail call and the CFG stays at exactly one region.
#[test]
fn g_is_single_region() {
    assert_eq!(region_count(&cfg("g")), 1, "g: expected 1 region (tail-call treated as exit)");
}
...
fn add_entry_region_branch_is_none() {
    let c = cfg("add");
    assert!(c.region_branch(c.entry).unwrap().is_none(), "add: linear entry should have no branch successor");
}
...
// ── global invariants across all functions ────────────────────────────────────
