//! CFG integration tests for AArch64 (`binary_tests/binary_test_aarch64`).
//!
//! Mirror of `cfg_x86.rs` using the AArch64 binary and Sleigh spec.
//! Build the binary with: `make -C binary_tests binary_test_aarch64`
...
fn binary() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/../../binary_tests/binary_test_aarch64")
}
...
fn cfg(fn_name: &str) -> cfg::Cfg<reader::ElfFileMemReader<'static, 'static>> {
    build_cfg(
        binary(),
        fn_name,
        rsleigh::sla_spec::SLA_SPEC_AARCH64,
        rsleigh::pspec::PSPEC_AARCH64,
    )
}
