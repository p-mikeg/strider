...
pub fn symbol_addr(binary_path: &str, fn_name: &str) -> u64 {
    let leaked: &'static [u8] =
        Box::leak(std::fs::read(binary_path).expect("read binary").into_boxed_slice());
...
pub fn build_cfg(
    ...
    let mem_reader = reader::ElfFileMemReader::from_elf_sections(parsed)
        .expect("build ElfFileMemReader");

    let sleigh = rsleigh::Sleigh::new(sla_spec, pspec, mem_reader)
...
pub fn all_edge_endpoints_valid<R: rsleigh::MemReader>(cfg: &Cfg<R>) -> bool {
    cfg.graph.edge_references().all(|e| {
        cfg.graph.node_weight(e.source()).is_some()
            && cfg.graph.node_weight(e.target()).is_some()
    })
}
