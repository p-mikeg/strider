...
fn x86_64_samples() -> (SleighArch, Vec<Sample>) {
    ...
    let mut samples = Vec::new();
    samples.push(Sample { label: "cmp rax, rbx".into(), bytes: cmp.to_vec() });
    let codes: &[(&str, u8)] = &[
        ("JO", 0x70),
        ("JNO", 0x71),
        ("JB/JNAE/JC", 0x72),
...
fn x86_samples() -> (SleighArch, Vec<Sample>) {
    ...
    let mut samples = Vec::new();
    samples.push(Sample { label: "cmp eax, ebx".into(), bytes: cmp.to_vec() });
    let codes: &[(&str, u8)] = &[
        ("JO", 0x70),
        ("JNO", 0x71),
        ("JB", 0x72),
...
fn arm_samples() -> (SleighArch, Vec<Sample>) {
    ...
    let mut samples = Vec::new();
    samples.push(Sample { label: "cmp r0, r1".into(), bytes: cmp.to_le_bytes().to_vec() });
...
fn mips_samples() -> (SleighArch, Vec<Sample>) {
    ...
    samples.push(Sample { label: "slt $4, $5, $6".into(), bytes: slt.to_be_bytes().to_vec() });
    // sltu $4, $5, $6 — funct=0x2B
    let sltu = 0x00A6_202Bu32;
    samples.push(Sample { label: "sltu $4, $5, $6".into(), bytes: sltu.to_be_bytes().to_vec() });
...
fn ppc_samples() -> (SleighArch, Vec<Sample>) {
    ...
    samples.push(Sample { label: "cmpw cr0, r3, r4".into(), bytes: cmpw.to_be_bytes().to_vec() });
    ...
    samples.push(Sample { label: "beq cr0, +4".into(), bytes: beq.to_be_bytes().to_vec() });
    ...
    samples.push(Sample { label: "bne cr0, +4".into(), bytes: bne.to_be_bytes().to_vec() });
    // blt cr0, +4: BO=12, BI=0 → 010000 01100 00000 00000000000001 0 0 = 0x41800004
    let blt = 0x4180_0004u32;
    samples.push(Sample { label: "blt cr0, +4".into(), bytes: blt.to_be_bytes().to_vec() });
    // bgt cr0, +4: BO=12, BI=1 → 010000 01100 00001 00000000000001 0 0 = 0x41810004
    let bgt = 0x4181_0004u32;
    samples.push(Sample { label: "bgt cr0, +4".into(), bytes: bgt.to_be_bytes().to_vec() });
    // bge cr0, +4: BO=4, BI=0 → 0x40800004
    let bge = 0x4080_0004u32;
    samples.push(Sample { label: "bge cr0, +4".into(), bytes: bge.to_be_bytes().to_vec() });
    // ble cr0, +4: BO=4, BI=1 → 0x40810004
    let ble = 0x4081_0004u32;
    samples.push(Sample { label: "ble cr0, +4".into(), bytes: ble.to_be_bytes().to_vec() });
