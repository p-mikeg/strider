...
#[test]
fn cfg_detach_collapses_var_and_mem_phi_then_validates() -> crate::opt::Result<()> {
    ...
    let (call_t, _, _) = b.build_call_other(0, "cpuid", None, &[], &[], &[], None, false)?;
    ...
    let (call_f, _, _) = b.build_call_other(0, "cpuid", None, &[], &[], &[], None, false)?;
...
#[test]
fn cfg_detach_removes_two_dead_predecessors_then_validates() -> crate::opt::Result<()> {
    let var = reg_vn(0x1000, 8);
    let mut b = RegisterSet::new().tracked(var).arg(var).build_fn()?;
    let entry = b.create_region()?;
...
#[test]
fn cfg_detach_visits_control_dead_but_data_reachable_region() -> crate::opt::Result<()> {
    let var = reg_vn(0x1000, 8);
    let mut b = RegisterSet::new().tracked(var).arg(var).build_fn()?;
    let entry = b.create_region()?;
    let true_r = b.create_region()?;
