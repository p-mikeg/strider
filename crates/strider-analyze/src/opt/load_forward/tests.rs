...
use crate::opt::{ConstantFold, OptimizerPipeline, PhiCollapse, RegionCollapse};
...
fn forward_through_long_chain_of_disjoint_stack_stores() -> Result<()> {
    ...
    let mut fg = strider_ir_test_utils::make_sp_fn(sp, |b, sp_val| {
        ...
        let target_addr = b.build_int_binary_operation(
            sp_val, zero, IntBinaryOp::Add, ValueType::I32,
        )?;
...
fn strict_does_not_forward_across_non_sp_intervening_store() -> Result<()> {
    ...
    let pipeline = crate::opt::test_support::standard_test_strict();
    pipeline.run(&mut fg, &crate::opt::OptCtx::empty())?;
...
fn permissive_forwards_across_const_intervening_store() -> Result<()> {
    ...
    let pipeline = crate::opt::test_support::standard_test_permissive();
    pipeline.run(&mut fg, &crate::opt::OptCtx::empty())?;
...
fn bail_on_call_between() -> Result<()> {
    ...
    let addr4b =
        b.build_int_binary_operation(sp_val2, four, IntBinaryOp::Add, ValueType::I64)?;
...
fn per_branch_stores_same_offset_do_not_forward_and_synthesize_no_phi() -> Result<()> {
    ...
    let sp = sp32_vn();
    let mut b = RegisterSet::new().tracked(sp).callee_saved(sp).stack_vn(sp).build_fn()?;
    ...
    let addr_t =
        b.build_int_binary_operation(sp_t, four_t, IntBinaryOp::Add, ValueType::I32)?;
    let a = b.build_int_const(0xAAu64, ValueType::I32)?;
    ...
    let addr_e =
        b.build_int_binary_operation(sp_e, four_e, IntBinaryOp::Add, ValueType::I32)?;
...
fn dominating_store_across_collapsible_merge_forwards_with_no_phi() -> Result<()> {
    ...
    let addr_en =
        b.build_int_binary_operation(sp_en, four_en, IntBinaryOp::Add, ValueType::I32)?;
...
fn phi_identical_values_no_new_phi() -> Result<()> {
    ...
    let addr_t =
        b.build_int_binary_operation(sp_t, eight_t, IntBinaryOp::Add, ValueType::I32)?;
    ...
    let addr_l =
        b.build_int_binary_operation(sp_l, twelve_l, IntBinaryOp::Add, ValueType::I32)?;
...
fn forwarding_bridges_sub_and_add_encodings_of_same_offset() -> Result<()> {
    ...
    let mut fg = strider_ir_test_utils::make_sp_fn(sp, |b, sp_val| {
        let four = b.build_int_const(4u64, ValueType::I32)?;
        let store_addr =
            b.build_sub_as_add_neg(sp_val, four, ValueType::I32)?;
...
fn narrow_load_from_wider_store_forwards_via_truncate() -> Result<()> {
    ...
    let mut fg = strider_ir_test_utils::make_sp_fn(sp, |b, sp_val| {
        let eight = b.build_int_const(8u64, ValueType::I32)?;
        let addr =
            b.build_sub_as_add_neg(sp_val, eight, ValueType::I32)?;
...
fn narrow_load_u16_from_u32_store_forwards_via_truncate() -> Result<()> {
    ...
    let mut fg = strider_ir_test_utils::make_sp_fn(sp, |b, sp_val| {
        let twelve = b.build_int_const(12u64, ValueType::I32)?;
        let addr =
            b.build_sub_as_add_neg(sp_val, twelve, ValueType::I32)?;
        let wide = b.build_int_const(0xDEAD_BEEFu64, ValueType::I32)?;
        b.build_store(addr, wide, rsleigh::VnSpace::RAM)?;
...
fn aborted_memphi_resolution_creates_no_nodes() -> Result<()> {
    ...
    // attribution).
    LoadForward::new().optimize(&mut fg, &crate::opt::OptCtx::empty())?;
