...
fn match_jump_table_shape_recognises_canonical_form() {
    ...
    let shape = match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor).expect("must match");
...
fn match_jump_table_shape_recognises_commuted_intadd() {
    ...
    let shape = match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor).expect("must match commuted add");
...
fn match_jump_table_shape_recognises_commuted_intmul() {
    ...
    let shape = match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor).expect("must match commuted mul");
...
fn match_jump_table_shape_recognises_both_commutations() {
    ...
    let shape = match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor).expect("must match both commuted");
...
fn match_jump_table_shape_recognises_shl_form() {
    ...
    let shape = match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor)
...
fn match_jump_table_shape_rejects_shl_with_oversize_shift() {
    ...
    assert!(
        match_jump_table_shape(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor).is_none(),
...
fn bound_via_known_bits_returns_max_plus_one() {
    ...
    let bound = bound_via_known_bits(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), idx, &known).expect("must bound");
    assert_eq!(bound, 8);
}
...
fn bound_via_known_bits_returns_none_for_unreachable_output() {
    ...
    let known = analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).expect("kb analyze");
    let bound = bound_via_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), detached_idx, &known);
    assert_eq!(
        bound, None,
...
fn bound_from_if_condition_idx_less_than_n_true() {
    ...
    let bound = bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, /* on_true */ true, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap());
...
fn bound_from_if_condition_idx_less_than_n_false_returns_none() {
    ...
    builder.set_lift_addr(None);
    let function = builder.build().unwrap();
    let bound = bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, /* on_true */ false, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap());
...
fn bound_from_if_condition_signed_less_unknown_sign_bit_returns_none() {
    ...
    let bound = bound_from_if_condition(
        ...
        idx,
        /* on_true */ true,
        &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap(),
    );
...
fn bound_from_if_condition_idx_le_n_true_is_n_plus_one() {
    ...
    builder.set_lift_addr(None);
    let function = builder.build().unwrap();
    let bound = bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, true, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap());
...
fn bound_via_predecessor_if_handles_deep_if_chain() {
    ...
    let bound = bound_via_predecessor_if(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), anchor, idx_in_dispatch, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap());
...
fn bound_via_predecessor_if_walks_one_hop() {
    ...
    let bound = bound_via_predecessor_if(strider_pattern::RewriteCtxView::from_built(&g).unwrap(), anchor, idx_in_dispatch, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&g).unwrap()).unwrap());
    assert_eq!(bound, Some(4));
}
...
fn bound_via_predecessor_if_returns_none_when_no_if_on_path() {
    ...
    let bound = bound_via_predecessor_if(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), anchor, idx, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap());
    assert_eq!(bound, None);
}
...
fn bound_from_if_condition_idx_equal_n_true_returns_none() {
    ...
    let function = builder.build().unwrap();
    assert_eq!(
        bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, /* on_true */ true, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap()),
        None,
        "Equal must NOT yield a 0..N bound — see H2 fix",
    );
    // Same on the false branch (the negation idx != N — also no bound).
    assert_eq!(bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, false, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap()), None);
}
...
fn bound_from_if_condition_with_n_on_lhs_does_not_match() {
    ...
    assert_eq!(bound_from_if_condition(strider_pattern::RewriteCtxView::from_built(&function).unwrap(), cmp, idx, true, &analyze_known_bits(strider_pattern::RewriteCtxView::from_built(&function).unwrap()).unwrap()), None);
