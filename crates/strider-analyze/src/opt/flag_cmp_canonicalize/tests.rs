...
fn flag_cmp_eq_rewrites_to_int_equal() -> Result<()> {
    ...
    let r = FlagCmpCanonicalize.optimize(&mut fg, &crate::opt::OptCtx::empty())?;
...
fn flag_cmp_hi_rewrites_after_constant_fold_runs_first() -> Result<()> {
    ...
    crate::opt::ConstantFold.optimize(&mut fg, &crate::opt::OptCtx::empty())?;
    let r = FlagCmpCanonicalize.optimize(&mut fg, &crate::opt::OptCtx::empty())?;
...
fn flag_cmp_thumb_beq_reduces_to_int_equal() -> Result<()> {
    ...
    let _ = FlagCmpCanonicalize.optimize(&mut fg, &crate::opt::OptCtx::empty())?;
    let _ = FlagCmpCanonicalize.optimize(&mut fg, &crate::opt::OptCtx::empty())?;
