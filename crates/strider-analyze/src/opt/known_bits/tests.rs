...
fn known_bits_or_then_and() -> Result<()> {
    ...
    while changed {
        let entry = fg2.entry();
        changed = KnownBits.optimize_raw(fg2.graph_mut(), entry)?.changed();
    }
...
fn known_bits_and_mask_then_and() -> Result<()> {
    ...
    while changed {
        let entry = fg.entry();
        changed = KnownBits.optimize_raw(fg.graph_mut(), entry)?.changed();
    }
...
fn known_bits_const_no_change() -> Result<()> {
    ...
    let entry = fg.entry();
    assert!(!KnownBits.optimize_raw(fg.graph_mut(), entry)?.changed());
    Ok(())
}
...
fn known_bits_neg_round_trip() -> Result<()> {
    let mut fg = make_fn(|b| {
        ...
        let n1 = b.build_int_unary_operation(or_, strider_ir::IntUnaryOp::BitNot, NodeOutputType::U8)?;
        b.build_int_unary_operation(n1, strider_ir::IntUnaryOp::BitNot, NodeOutputType::U8)
    })?;
    let mut changed = true;
    while changed {
        let entry = fg.entry();
...
fn make_fn_with_var<F>(f: F) -> Result<strider_ir::BuiltFunctionGraph>
where
