...
fn reassoc_no_fold_without_const() -> Result<()> {
    ...
    let entry = fg.entry().unwrap();
    let res = ConstantFold.optimize(&mut fg, entry)?;
...
fn distribution_rewrite() -> Result<()> {
    ...
    let mut fg = b.build()?;
    let entry = fg.entry().unwrap();
    let changed = ConstantFold.optimize(&mut fg, entry)?.changed();
...
fn fold_lzcount_u128_input_skips_cleanly() -> Result<()> {
    ...
    let entry = fg.entry().unwrap();
    let result = ConstantFold.optimize(&mut fg, entry);
    assert!(
        result.is_ok(),
...
fn single_pass_propagates_through_chain() -> Result<()> {
    ...
    let entry = fg.entry().unwrap();
    ConstantFold.optimize(&mut fg, entry)?;
