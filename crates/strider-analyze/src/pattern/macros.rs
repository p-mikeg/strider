...
//!
//! Each constructor returns a [`crate::pattern::pat::Pat`] (or a typed builder for
//! the binary-op families).  Buildable patterns route through the
//! `with_build_*` helpers in [`crate::pattern::pat::node_pat::NodePat`] so the
...
macro_rules! decl_pat_binary_ops {
    ($builder:ident, $op_enum:ident, $ret:ty, [ $( $(#[$attr:meta])* ($fn_name:ident, $variant:ident) ),* $(,)? ]) => {
        $(
            $(#[$attr])*
            pub fn $fn_name(lhs: impl Into<$crate::pattern::pat::Pat>, rhs: impl Into<$crate::pattern::pat::Pat>) -> $ret {
...
macro_rules! decl_pat_unary_ops {
    ($builder:ident, $op_enum:ident, $ret:ty, [ $( $(#[$attr:meta])* ($fn_name:ident, $variant:ident) ),* $(,)? ]) => {
        $(
            $(#[$attr])*
            pub fn $fn_name(operand: impl Into<$crate::pattern::pat::Pat>) -> $ret {
...
// [`crate::pattern::int_const_with_fn`] (or bool/float variants) with a closure that
...
//
//   * `ty` — the root node's output type ([`crate::pattern::BuildCtx::root_ty`]).
...
macro_rules! __const_with_bind_one {
    ...
    ($ctx:ident, in_ty, $hy:ident) => {
        let $hy = $crate::pattern::first_value_input_type($ctx);
        let _ = &$hy;
    };
}
