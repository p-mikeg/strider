...
//!
//! Only one helper remains: [`next_unique_consumer`] resolves the single
...
pub(crate) fn next_unique_consumer(matcher: &Matcher, out: NodeOutputId) -> Option<NodeId> {
...
mod tests {
    ...
    #[test]
    fn next_unique_consumer_returns_single_consumer() -> strider_ir::Result<()> {
        ...
        let got = next_unique_consumer(&m, entry_ctrl_out(&function)).expect("one consumer");
        assert!(matches!(function.node_kind(got), NodeKind::Region));
        Ok(())
    }

    #[test]
    fn next_unique_consumer_returns_none_when_no_consumer() -> strider_ir::Result<()> {
        ...
        let m = Matcher::try_new(&function).unwrap();
        assert_eq!(next_unique_consumer(&m, out), None);
        Ok(())
    }
}
