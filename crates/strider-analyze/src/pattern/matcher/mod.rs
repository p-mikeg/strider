...
/// [`Matcher::find_joined`].
///
/// A shared [`Capture`] must bind the same
/// [`crate::pattern::matcher::bindings::Binding`] across matches.  Captures
/// local to `m` (not seen in `prefix`) impose no constraint — they are
/// unique to this pattern and join-trivial.
