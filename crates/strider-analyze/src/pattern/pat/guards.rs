...
//! [`crate::pattern::pat::IntoPat::when`] and [`crate::pattern::pat::Pat::when_match`].
...
///   common [`crate::pattern::pat::IntoPat::when`] combinator.
/// * [`Self::Bindings`] sees the full capture [`Bindings`] map — needed by
///   guards that cross-reference multiple captures
///   ([`crate::pattern::pat::Pat::when_match`]).
pub enum GuardFn {
    Output(crate::pattern::pat::PredicateFn),
    Bindings(crate::pattern::pat::MatchPredicateFn),
}

/// Post-match predicate combinator.  `inner` is a [`crate::pattern::pat::Pat`] so the
/// fluent builder API (`impl Into<Pat>`) can wrap arbitrary patterns
/// uniformly; dispatch goes through [`crate::pattern::matcher::Matcher::match_output`].
///
...
/// [`GuardFn`] variants require a [`strider_ir::node::NodeOutputType`] (sourced
