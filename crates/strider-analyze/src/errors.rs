...
/// Raised by the strider lifter when a `pcode::CallOther` opcode's user-op
/// `name` has no entry in [`strider_target::call_other_abi::classify`].
/// Strict-on-emission policy: any new user-op surfaced by a real lift
/// must be classified before the lift can succeed.
///
