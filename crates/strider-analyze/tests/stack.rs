...
fn volatile_preserves_three_stores(function: &strider_ir::Function) {
    ...
    assert!(count_stores(function) >= 3,
            "volatile must preserve 3 stores; got {}", count_stores(function));
}
...
fn swap_has_two_loads_and_two_stores(function: &strider_ir::Function) {
    assert!(count_loads(function) >= 2, "expected ≥2 Loads; got {}", count_loads(function));
    assert!(count_stores(function) >= 2, "expected ≥2 Stores; got {}", count_stores(function));
}
