...
fn array_copy_has_load_and_store(function: &strider_ir::Function) {
    assert!(count_loads(function) >= 1, "array_copy must Load");
    assert!(count_stores(function) >= 1, "array_copy must Store");
    assert!(count_loops(function) >= 1);
}
fn pointer_chase_has_two_loads(function: &strider_ir::Function) {
    assert!(count_loads(function) >= 2, "pointer_chase has 2 indirections; got {}", count_loads(function));
}
fn struct_load_has_load(function: &strider_ir::Function) {
    assert!(count_loads(function) >= 2, "x and y are two field reads; got {}", count_loads(function));
}
fn struct_store_has_store(function: &strider_ir::Function) {
    assert!(count_stores(function) >= 2, "x and y are two field writes; got {}", count_stores(function));
}
fn union_read_has_load(function: &strider_ir::Function) {
    ...
    assert!(count_loads(function) >= 1, "union read must Load at least once; got {}", count_loads(function));
}
