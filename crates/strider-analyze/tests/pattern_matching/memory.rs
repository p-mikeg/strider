...
fn load_addr_matches_literal() {
    let g = shapes::store_then_load_ram(0x100, 42);
    a::matches(&g, load().addr(int_const(0x100u64)), 1);
    a::none(&g, load().addr(int_const(0x999u64)));
}
...
fn load_captures_value_slot() {
    let g = shapes::store_then_load_ram(0x100, 42);
    let v = Capture::new();
    let m = a::unique(&g, load().addr(int_const(0x100u64)).capture(v));
...
fn store_addr_matches() {
    let g = shapes::store_then_load_ram(0x100, 42);
    a::matches(&g, store().addr(int_const(0x100u64)), 1);
    a::none(&g, store().addr(int_const(0x999u64)));
}

#[test]
fn store_data_matches() {
    let g = shapes::store_then_load_ram(0x100, 42);
    a::matches(&g, store().data(int_const(42u64)), 1);
    a::none(&g, store().data(int_const(1u64)));
}
...
fn store_then_load_same_addr_match() {
    ...
    a::matches(&g, load().addr(int_const(0x200u64)), 1);
}
