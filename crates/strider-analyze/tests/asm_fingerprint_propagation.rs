...
fn constant_fold_add_consts_preserves_fingerprints() {
    ...
    .unwrap();
    assert!(ConstantFold::new().optimize(&mut fg, &strider_analyze::opt::OptCtx::empty()).unwrap().changed());
...
fn known_bits_fold_preserves_fingerprints() {
    ...
    .unwrap();
    let _ = ConstantFold::new().optimize(&mut fg, &strider_analyze::opt::OptCtx::empty());
    let _ = KnownBits.optimize(&mut fg, &strider_analyze::opt::OptCtx::empty());
...
fn constant_fold_and_mask_merge_preserves_fingerprints() {
    ...
    .unwrap();
    let _ = ConstantFold::new().optimize(&mut fg, &strider_analyze::opt::OptCtx::empty()).unwrap();
