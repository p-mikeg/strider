...
///
/// **Phase 8c — re-enabled**: `optimized_function` now drives the
/// per-region queries to produce a content-key for the BFG cache
/// side-table (see `orchestrator_salsa::optimized_function` body).
/// The first query populates both the per-region and the content
/// caches; subsequent queries with unchanged signatures hit both.
#[test]
...
///
/// **Phase 8c — re-enabled**: see `first_query_invokes_one_signature_per_region`.
#[test]
