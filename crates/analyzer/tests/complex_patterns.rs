...
use pattern::{
    CastMask, IntCmpOp, IntVar, Matcher, NodeVar, Pat, Var,
...
fn bit_test_against_zero(value: Var, mask_var: IntVar) -> Pat {
    int_cmp(
        IntCmpOp::Equal,
...
fn field_load_at_offset(base: Var, offset: IntVar) -> Pat {
...
fn read_struct_fields_assertions(g: &ir::BuiltFunctionGraph) {
    ...
    let offsets: Vec<u128> = hits.iter().filter_map(|h| h.get_int(off)).collect();
...
fn write_struct_fields_assertions(g: &ir::BuiltFunctionGraph) {
...
fn nested_struct_field_assertions(g: &ir::BuiltFunctionGraph) {
...
fn bit_test_zero_assertions(g: &ir::BuiltFunctionGraph) {
    ...
    assert!(count_int_binop(g, ir::IntBinaryOp::And) >= 1,
            "bit_test_zero must contain ≥1 IntBinaryOp::And; got {}",
            count_int_binop(g, ir::IntBinaryOp::And));
    assert!(count_int_cmp(g, ir::IntCmpOp::Equal) >= 1,
            "bit_test_zero must contain ≥1 IntCmpOp::Equal; got {}",
            count_int_cmp(g, ir::IntCmpOp::Equal));
...
fn if_bit_clear_call_assertions(g: &ir::BuiltFunctionGraph) {
...
fn call_with_field_arg_assertions(g: &ir::BuiltFunctionGraph) {
    assert!(count_loads(g) >= 1,
...
fn dispatch_on_flag_assertions(g: &ir::BuiltFunctionGraph) {
...
fn multi_arg_call_in_branch_assertions(g: &ir::BuiltFunctionGraph) {
    ...
    let abc_ids: std::collections::HashSet<_> =
        hits_abc.iter().filter_map(|h| h.get_node(nv_abc)).collect();
    let cba_ids: std::collections::HashSet<_> =
        hits_cba.iter().filter_map(|h| h.get_node(nv_cba)).collect();
...
fn complex_dispatch_assertions(g: &ir::BuiltFunctionGraph) {
    ...
    let offsets: std::collections::HashSet<u128> = m
        .find_all(&pat).iter().filter_map(|h| h.get_int(off)).collect();
...
fn call_uses_call_return_assertions(g: &ir::BuiltFunctionGraph) {
