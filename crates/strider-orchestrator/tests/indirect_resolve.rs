//! Tests for [`strider_orchestrator::indirect_resolver::resolve_indirect_target`] —
...
//! `crates/strider-orchestrator/src/indirect_resolver.rs`; the integration
    ...
    while cur < start + total_len {
        ...
        for (i, insn) in lift.insns.iter().enumerate() {
            out.push(RegionInstruction {
                addr: PcodeInsnAddr {
                    machine_addr: MachineInsnAddr::from(cur),
                    insn_index: i as u64,
                },
                insn: insn.clone(),
            });
