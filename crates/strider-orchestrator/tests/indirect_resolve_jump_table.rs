...
/// `idx & 0x7; load[base + idx*stride]; jmp *target` -> `Multiple([table[0],
/// ..., table[7]])`. `KnownBits` proves `idx <= 0x7` from the AND-mask, so
/// the bound is 8; the classifier reads 8 entries and returns a canonical
/// (sorted, deduplicated) Multiple.
#[test]
...
/// Same shape as the KnownBits test but the index has no AND-mask and no
/// predecessor If. Must return `None`: a `Multiple` here would be unsound
/// since the runtime index could be any I32 value, and we'd be enumerating
/// an arbitrary prefix of memory as "targets".
#[test]
...
/// Bounded shape, but `rom = None`: without rom we can't read entries, so
...
fn jump_table_no_rom_returns_none() {
    ...
    let idx_mask = 0x3u64;
    let (function, _target) = build_jump_table_known_bits_scenario(base, stride, idx_mask);
    let result = classify_target_with_rom(&function, None).expect("classify_target_with_rom");
    assert_eq!(result, None);
}

/// Bounded shape, rom covers some but not all entries. Must return `None`:
/// a partial Multiple would induce a CFG missing real edges.
#[test]
...
/// Pinned: when the proved bound is zero, the classifier returns `None`
/// rather than `Multiple([])`. A zero upper bound only proves the branch is
/// unreachable if we separately proved unreachability, which we haven't;
/// misclassifying a reachable branch as dead would silently truncate the
/// CFG. `None` defers to a later iteration or to `UnresolvedIndirectBranch`
/// at fixed point.
///
/// Trigger: a predecessor-If with bound = 0 (`if (idx < 0)`, dead by
/// construction for unsigned idx); the classifier must still answer
/// cleanly rather than mishandle the degenerate bound.
#[test]
...
// The FunctionBuilder fixtures above hand-build the guard in a
// region-built shape. These tests instead drive real x86-64 bytes through
...
// guard polarities:
//
//   * jb polarity: `cmp rdi, N; jb .dispatch` (branch toward dispatch on
//     true). Post-pipeline cond is the directly-recognised
//     `Less(rdi, IntConst(N))` with the dispatch on the true edge.
//   * ja polarity: `cmp rdi, N-1; ja .default` (branch away on true, the
//     shape gcc/clang emit for `switch`).
//
// Both resolve through the default pipeline:
//   1. (both) `RegionCollapse` removes the single-predecessor dispatch
//      Region so the If's control feeds the `IndirectBranch` placeholder
//      directly. `compute_value_ranges` keys each guard by the unique
//      control consumer of the guarded If-edge, so the guard's dominance
//      survives the collapse.
//   2. (both) the guard is modelled on both If edges and for a constant on
//      either operand of `Less`/`Sless`, so the ja bound (`idx <= N` on the
//      false edge of `N < idx`, or the true edge after `IfCondInversion`)
//      is recognised.
//   3. (ja only) `FlagCmpCanonicalize` rule 14 folds the constant-cmp ja
//      `CF||ZF` tree (`ConstantFold` first folds `Neg(IntConst(N))` to
...
/// Asserts the analyze result resolved the table dispatch to EXACTLY
/// [`TABLE_TARGETS`]: nothing in `unresolved_indirect_branches`, no
/// `IndirectBranch` placeholder left in the returned IR, and one `Switch`
/// carrying all four arms. Without the arm check a one-arm answer, or four
/// wrong addresses, reads as a clean resolution.
fn assert_dispatch_resolved(result: &strider_orchestrator::AnalyzeResult, what: &str) {
    ...
    assert_eq!(
        arms,
        vec![TABLE_TARGETS.to_vec()],
        "{what}: the seated arms must be the whole table",
    );
}
...
/// 0x1010..0x1013: c3 x4            table targets
/// ```
#[test]
...
/// `Or(Less(rdi, 3), Equal(Add(rdi, -3), 0))` to `rdi <= 3`; after
/// `IfCondInversion` the dispatch sits on the post-pipeline If's true edge
/// with the bound the range analysis reads (see the section comment above).
///
/// ```text
...
/// IntConst))`, e.g. `Load(IntConst(addr))` for a simple global read. Must
/// fall through to `None`: it's neither a jump-table dispatch nor a
/// recognised non-Load shape.
#[test]
