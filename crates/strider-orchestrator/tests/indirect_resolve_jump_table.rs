...
//! see), then invokes [`classify_target`] on the placeholder
...
/// calls `classify_target` on the fixture's sole `IndirectBranch` placeholder
/// (the classifier derives the dispatch target from the branch's slot-2 input).
