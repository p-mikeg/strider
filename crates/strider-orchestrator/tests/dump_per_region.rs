...
use common::dump_helpers::{
    ScratchDir, VIEWER_JSON_ANCHOR, lift_branch_snippet_x86_64, lift_ret_snippet_x86_64,
};
...
fn dump_per_region_writes_one_html_per_region() {
    ...
    strider_orchestrator::dump_per_region(
        &outcome.function,
...
fn dump_per_region_rejects_post_compaction() {
    ...
    let lift_gen = outcome.function.graph().generation();
    ...
    let err = strider_orchestrator::dump_per_region(
        &outcome.function,
        exit_controls.iter().copied(),
        lift_gen,
        &sleigh,
