...
//! [`strider_orchestrator::opt::inplace`].
//!
...
fn apply_link_register_to_real_lift_zero_ret_vals_drops_target_value() {
    ...
    function
        .with_rewrite_ctx(|ctx| apply_link_register(ctx, return_id, &[]))
        .expect("apply");
    ...
    assert_eq!(
        inputs_after.len(),
        2,
        "target_value dropped, no ret_vals appended"
    );
...
fn apply_link_register_to_real_lift_appends_one_ret_val() {
    ...
    let inputs_before: Vec<_> = function.node_inputs(return_id).into_iter().collect();
    function
        .with_rewrite_ctx(|ctx| apply_link_register(ctx, return_id, &[anchor]))
        .expect("apply");
    let new_return = locate_fresh_return(&function);
    let inputs_after: Vec<_> = function.node_inputs(new_return).into_iter().collect();
    assert_eq!(
...
fn apply_tail_call_replaces_placeholder_with_call_then_return() {
    ...
    let new_return = function
        .with_rewrite_ctx(|ctx| {
            apply_tail_call(ctx, return_id, target, anchor, &[], &[], &[], false)
        })
...
fn apply_tail_call_returns_node_id_of_new_return() {
    ...
    let new_return = function
        .with_rewrite_ctx(|ctx| {
            apply_tail_call(ctx, return_id, 0xc0de_u64, anchor, &[], &[], &[], false)
        })
        .expect("apply_tail_call");
...
fn apply_tail_call_new_return_control_input_is_call_output() {
    ...
    let new_return = function
        .with_rewrite_ctx(|ctx| {
            apply_tail_call(ctx, return_id, 0xface_u64, anchor, &[], &[], &[], false)
        })
        .expect("apply_tail_call");
...
fn apply_link_register_emits_fresh_return_and_detaches_placeholder() {
    ...
    assert_eq!(
        function.node_inputs(return_id).len(),
        0,
...
fn apply_tail_call_real_lift_target_int_const_value_matches() {
    ...
    let call_inputs: Vec<_> = function.node_inputs(call_node).into_iter().collect();
...
fn apply_tail_call_with_calling_context_exposes_arg_slot_0_to_pattern_query() {
    ...
    use strider_pattern::{Capture, CaptureExt, Matcher, any, call};
    ...
    let call_node = {
        ...
        let ret_inputs: Vec<_> = function.node_inputs(_new_return).into_iter().collect();
...
fn apply_tail_call_with_preserves_memory_wires_pre_call_memory_into_return() {
    ...
    let placeholder_mem_value = function
        .graph()
        .nth_input(return_id, 1)
    ...
    let new_mem_value = function
        .graph()
        .nth_input(new_return, 1)
        .expect("Return mem input");
    assert_eq!(
    ...
    let ret_ctrl_value = function
        .graph()
        .nth_input(new_return, 0)
        .expect("Return ctrl input");
...
fn apply_tail_call_without_preserves_memory_threads_call_memory_into_return() {
    ...
    assert_eq!(
        new_mem_value, call_outs[1],
        "Return mem must equal Call's Memory output"
    );
    // Full validate skipped — editor-isolation test, see note above.
}
