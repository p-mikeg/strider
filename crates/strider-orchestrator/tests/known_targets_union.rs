...
//! round, so a seed never suppresses what the classifier proves about a site.
//! Seating it is all-or-nothing: the seed asserts the site's successor set is
//! complete, so one member the cfg cannot seat leaves the whole site unseated
//! and reported unresolved.
...
fn arms_with(seed: Option<Vec<u64>>) -> (Vec<u64>, Vec<PcodeInsnAddr>, Vec<u64>) {
    ...
    starts.dedup();
    (arms, r.unresolved_indirect_branches, starts)
}
...
fn a_seed_the_cfg_declines_is_reported_unresolved_not_partially_seated() {
    let (baseline, _, starts) = arms_with(None);
    ...
    let (good_seed, _, _) = arms_with(Some(vec![extra]));
    assert!(
    ...
    let (mixed, unresolved, _) = arms_with(Some(vec![extra, OUT_OF_RANGE]));
    assert!(
        !unresolved.is_empty(),
        "the unseatable seed member {OUT_OF_RANGE:#x} vanished silently: the \
         site was reported fully resolved",
    );
    assert!(
