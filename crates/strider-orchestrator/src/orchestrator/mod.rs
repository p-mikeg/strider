...
fn apply_in_place_edit(
    ...
    match resolved {
        ResolvedTargets::LinkRegister => {
            let ctx =
                anchor_calling_context_for(function, placeholder, strider, region_index, None)?;
        ...
        ResolvedTargets::Single(target) => {
            ...
            let ctx = anchor_calling_context_for(
                function,
                placeholder,
                strider,
                region_index,
            ...
            // this list come from the same `anchor_calling_context_for` projection over
            // `function.variables`).
                ...
                && let Some(call_id) = locate_spliced_call(function.graph(), new_return)
            {
                ...
                let clobber_outputs: Vec<ValueId> = function
...
fn anchor_calling_context_for(
    ...
    let cc: &strider_target::BuiltCallingConvention =
        override_cc.unwrap_or_else(|| strider.calling_convention());
...
/// [`anchor_calling_context_for`]'s clobber
