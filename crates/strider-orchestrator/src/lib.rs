    ...
    R: rsleigh::MemReader,
{
    ...
    /// and every arch with no ISA-mode var.
    fn flowing_isa_bits(
        &self,
    ...
    pub fn analyze(
        ...
        // The mode a target seated WITHOUT one decodes in. `Builder::
        // enqueue_resolved` reads it at the branch address, not at the entry, so
        // a `mov pc` in a Thumb region of an otherwise-ARM function keeps Thumb;
        // reading it per site is what makes the fold's interworking test agree
        // with the decode. The entry mode is the fallback for a site whose
        // context has not been painted yet.
        let entry_isa_bit = self
            .arch
        ...
        // re-unions the seed every round. A seed asserts the site is settled, so
        // it suppresses the report of a successor carried over from an earlier
        // round; it does not suppress a loss of what the classifier derives, nor
        // a live placeholder.
        ...
        for _ in 0..MAX_RESOLUTION_ITERATIONS {
            ...
            let flowing = self.flowing_isa_bits(&unresolved, &switch_anchors, entry_isa_bit);
            let progress = apply_resolutions(
                &mut working.cfg.known_targets,
                &lift_opts.cfg.known_targets,
                &unresolved,
                &switch_anchors,
                // Kept for the post-loop fold: a `break` leaves the round's
                // classifications unfolded, and one entry per indirect site is
                // a cheap clone.
                resolutions.clone(),
                &flowing,
            )?;
        ...
        if !converged {
            let mut final_targets = working.cfg.known_targets.clone();
            let flowing = self.flowing_isa_bits(&unresolved, &switch_anchors, entry_isa_bit);
            let progress = apply_resolutions(
                &mut final_targets,
                &lift_opts.cfg.known_targets,
            ...
            oscillating |= progress
                .narrowed
                .iter()
                .any(|addr| narrow_rounds.get(addr).is_some_and(|seen| *seen > 0));
            incomplete.extend(progress.incomplete);
            derived_incomplete.extend(progress.derived_incomplete);
            still_growing = progress.grew;
        }
...
struct Progress {
    ...
    /// Addresses whose fold dropped a successor carried over from a previous
    /// round or from the caller's own seed. A seed asserts the site is
    /// complete, so this channel is the one a seed suppresses.
...
fn apply_resolutions(
    ...
    for (addr, targets) in staged {
        ...
        let carried_dropped = prev_targets.iter().any(|t| !seated.contains(&t.addr));
...
/// The three non-placeholder channels `live_unresolved_branches` merges.
#[derive(Clone, Copy)]
struct DerivedChannels<'a> {
    /// Sites still growing when the iteration cap ran out.
    budget_exhausted: &'a [PcodeInsnAddr],
    /// Sites whose fold dropped a successor carried over from an earlier round
    /// or from the caller's seed.
    incomplete: &'a [PcodeInsnAddr],
    /// Sites whose fold dropped a successor the classifier proved, or whose
    /// answer stopped being a superset.
...
/// - `incomplete`, the sites whose fold dropped a successor carried over from
///   an earlier round or from the caller's own seed.
/// - `derived_incomplete`, the sites whose fold dropped a successor the
///   classifier proved THIS round, or whose answer stopped being a superset.
///
/// A caller seed asserts the site is complete, which suppresses the middle two
/// channels. It does NOT suppress a live placeholder: the cfg re-emits one
...
/// that site. It does NOT suppress `derived_incomplete` either: a seed is an
/// answer about the site, not a licence to drop what the classifier proves
/// about it.
