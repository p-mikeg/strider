...
/// table, and the [`strider_opt::AliasMode`] used by the SP-aware
...
impl LiftDriver {
    ...
    pub(crate) fn find_all_unique_vns(&self, cfg: &strider_lift::cfg::Cfg) -> Vec<rsleigh::Vn> {
    ...
    pub fn analyze_cfg_with<R: rsleigh::MemReader>(
        ...
        let ir_region_of =
            |region_id: strider_lift::cfg::RegionId| -> Result<strider_ir::RegionId> {
                region_map
                    .get(region_id.index())
                    .copied()
                    .flatten()
...
fn init_region_map<R: rsleigh::MemReader>(
    ...
    Vec<strider_lift::cfg::RegionId>,
    Vec<Option<strider_ir::RegionId>>,
)> {
...
impl SpecialTerm {
    fn from_terminator(t: &strider_lift::cfg::RegionTerminator) -> Option<Self> {
        match t {
            ...
            strider_lift::cfg::RegionTerminator::Switch { target_vn, targets } => {
                Some(SpecialTerm::Switch(*target_vn, targets.clone()))
            }
            strider_lift::cfg::RegionTerminator::TailCall { target } => {
                Some(SpecialTerm::TailCall(*target))
            }
            _ => None,
        }
