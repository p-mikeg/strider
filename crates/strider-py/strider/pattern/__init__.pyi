...
class Match:
    ...
    def vn(self, key: CaptureKey) -> Vn:
        ...
        return-value or clobber output). Raises when `key` binds neither."""
        ...
...
class Pat(OrderedPat):
    ...
    `.when`, `.ordered`, `.of_width`, `.value_ty`, `.bool_valued` and `.const`.
...
class NodePat(Protocol):
    ...
    def when(self: _S, f: Callable[[Match], bool]) -> _S:
        ...
        backtracks, so other bindings are still tried."""
        ...
...
#: `CallPat.output(slot)` returned this name in 0.2.0; it is the shared
#: terminal now.
CallOutputPat = OutputSlotPat

class IntBinaryPat(NodePat, OrderedPat):
    """Builder for `any_int_binary(c, l, r)`: any `IntBinaryOp` variant, with
    the op name bound to `c`."""
class FloatBinaryPat(NodePat, OrderedPat):
    """Builder for `any_float_binary(c, l, r)`: any `FloatBinaryOp` variant,
    with the op name bound to `c`."""
class BoolBinaryPat(NodePat, OrderedPat):
    """Builder for `any_bool_binary(c, l, r)`: any boolean binary op, with the
    op name bound to `c`.
