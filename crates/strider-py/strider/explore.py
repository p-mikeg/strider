...
def shutdown(port: int | None = None) -> list[int]:
    ...
    A thread still parked in `visualize` when the interpreter shuts down is
    killed in a way strider cannot recover from, and the process aborts, so
    any caller that started an explorer off the main thread must shut it
    down.
    """
