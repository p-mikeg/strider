...
class LoadPat:
    ...
    `Load` is a single-output node (the loaded value); there is no
    forward `.next_mem` because Load doesn't produce a memory edge.
    """
...
class StorePat:
    ...
    `.bit_width(n)`, `.space(s)`.  `.next_mem(p)` walks forward to the
    unique consumer of the store's memory output (no match if zero or
    multiple consumers).  `.bit_width(n)` filters by the data input's
    width.
    """
...
# Phase 4 Task 4.0 — V2 reference type emitted by `#[gen_stub_pyclass]`
# + `#[gen_stub_pymethods]`.  The signature here is the exact shape
# `examples/stub_gen.rs` produces under
# `crates/strider-py/strider/_generated/strider/pattern.pyi` — the
# difference is that hand-written stubs ship in the wheel while the
# generated one is dev-only (gitignored).  Task 4.2 will replace this
# hand-written block with the auto-generated one once the macro
# covers every typed builder.
class StackStorePatV2:
    """V2 reference for `StackStorePat`.  Macro-emission shape for
    Task 4.1's `#[strider_pattern]` proc-macro.

    Chain `.offset(k)`, `.offset_any(set)`, `.data(p)`, `.space(s)`,
    `.capture(c)` / `.cap(name)`, `.when(predicate)`.
    """
    def __init__(self) -> None: ...
    def offset(self, k: int) -> "StackStorePatV2": ...
    def offset_any(self, offsets: set[int]) -> "StackStorePatV2": ...
    def data(self, p: PatLike) -> "StackStorePatV2": ...
    def space(self, s: object) -> "StackStorePatV2": ...
    def capture(self, c: Capture) -> "StackStorePatV2": ...
    def cap(self, name: str) -> "StackStorePatV2": ...
    def when(self, f: Callable[[PartialMatch], bool]) -> "StackStorePatV2": ...
    def into_pat(self) -> Pat: ...
