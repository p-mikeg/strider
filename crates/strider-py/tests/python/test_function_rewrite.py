...
def _build_graph(symbol="array_sum"):
    # Unoptimized so the raw `int_add(a, b)` shapes survive for a rewrite to fire.
...
def test_clone_is_independent_of_original():
    ...
    assert before > 0, "fixture must contain at least one int_add(a, b)"

    clone = g.clone()
    # A structural twin at first.
    assert len(clone.find_all(int_add(var(x), var(y)))) == before

    fired = clone.rewrite(find=int_add(var(x), var(y)), replace=tpl.var(x))
    assert fired > 0, "the rewrite must mutate the clone"

    after_original = len(g.find_all(int_add(var(x), var(y))))
    assert after_original == before, "rewriting the clone must not touch the original"

    after_clone = len(clone.find_all(int_add(var(x), var(y))))
...
def test_rewrite_returns_zero_when_nothing_matches():
    ...
    fired = clone.rewrite(
        find=int_add(int_const(sentinel), int_const(sentinel)),
...
def test_clone_match_staleness_is_independent():
    ...
    orig_hits = g.find_all(int_add(var(x), var(y)))
    clone_hits = clone.find_all(int_add(var(x), var(y)))
