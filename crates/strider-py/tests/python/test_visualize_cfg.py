...
def test_interpreter_exit_with_a_parked_explorer_does_not_abort():
    """An explorer left serving at interpreter exit must not take the process
    down.

    `shutdown` alone was not enough: it returns as soon as the serve loop
    exits, while the thread is still unwinding out of the Rust frame, and
    finalizing then aborts (SIGABRT, exit 134). The `atexit` hook joins the
    thread, so the interpreter waits for it to leave that frame.

    Run in a subprocess because the failure is a process-level abort, which no
    ...
    script = textwrap.dedent(
        """
        import threading, time, strider, strider.explore
        def run():
            lift = strider.lift.lifter(
                strider.sleigh.SleighArch.x86_64(),
                strider.reader.BufferReader(0x1000, b"\\x75\\x01\\x90\\xc3"),
            )
            lift.visualize(lift.build_cfg(0x1000), port=0)
    ...
    assert proc.returncode == 0, (
        f"interpreter exit aborted (rc={proc.returncode}); "
        f"stderr:\n{proc.stderr.decode(errors='replace')[-2000:]}"
    )


def test_a_non_daemon_explorer_thread_does_not_hang_interpreter_exit():
    """The exit hook must run BEFORE the non-daemon-thread join, not after.
    ...
    script = textwrap.dedent(
        ...
        def run():
            ...
            lift.visualize(lift.build_cfg(0x1000), port=0)
        threading.Thread(target=run).start()   # non-daemon on purpose
        time.sleep(2)
        """
    )
    try:
        ...
        raise AssertionError(
            "interpreter exit hung: the shutdown hook did not run before the "
