...
def test_default_pipeline_mirrors_rust_default():
    ...
    a graph that doesn't canonicalise flag-cmp shapes, so pattern queries
    that work under the orchestrator path (which uses the Rust default)
    fail under the custom-pipeline path.
    """
    assert strider.OptimizerPipeline.default().pass_count() == 11
    assert strider.OptimizerPipeline.default().post_pass_count() == 3
