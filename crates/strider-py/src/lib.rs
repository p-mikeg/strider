...
fn force_anyhow_backtrace_capture() {
        ...
        && std::env::var_os("RUST_BACKTRACE").is_none()
    {
        ...
        // `getenv` is reading, handing that reader a dangling pointer; hence
        // the `unsafe`.  This runs from `#[pymodule]` init, serialised by
        // Python's import lock and before this extension starts any thread.
        unsafe {
