...
impl PyLifter {
    ...
    fn analyze(
        ...
        let assume_incoming_args_survive_calls = opts_ref.assume_incoming_args_survive_calls;
        let resolve_indirect_branches = opts_ref.resolve_indirect_branches;
        let assumptions = {
            let a = opts_ref.assumptions.borrow(py);
    ...
    /// stays usable.  Invalidates outstanding `Node` / `Match` handles for
    /// `function`.
    ...
    fn optimize(
        ...
        with_pending_control_flow(|| match pipeline {
            Some(p) => function.run_pipeline_in_place(p.build_pipeline(), "optimize"),
            None => {
