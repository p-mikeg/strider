...
impl<'a, R: rsleigh::MemReader> FunctionLifter<'a, R> {
    ...
    fn build_cc_call(
        ...
        let (ret_vns, clobber_vns, arg_vns, float_arg_vns, ret_stack_pop) = {
            ...
            //
            // Truncated at the first untracked ABI position, keeping every
            // emitted argument at its own index: a float argument register the
            // function never names holds no value to pass, and the ABI fills
            // the file densely, so the registers after it are scratch.
