...
impl<'a, R: rsleigh::MemReader> FunctionLifter<'a, R> {
    ...
    pub(crate) fn handle_switch(
        ...
        for &target in targets {
            ...
            let cfg_region = self
                .cfg
                .switch_arm_region(region_id, machine_addr)
                .ok_or_else(|| {
    ...
    fn build_cc_call(
        ...
        let (ret_vns, clobber_vns, arg_vns, float_arg_vns, ret_stack_pop) = {
            ...
            // Float / vector arguments live in a register file the integer list
            // never names, so without them the callee's float argument cone has
            // no consumer and DCE deletes it.  Registers sharing a container
            // (AAPCS-VFP `d0`/`d1` inside `q0`) each pass their own slice of
            // it, so one argument is never two.
            //
            // Every convention reaching here has its float argument registers
            // seeded into the tracked set, so a `None` slot means a caller built
            // a convention this function was not lifted against. Emitting the
            // prefix before the first gap keeps index `j` meaning ABI position
            // `j`, which is what the callee side records; flattening past a gap
            // shifts every later argument down one.
            let float_arg_vns: Vec<rsleigh::Vn> = cc
                .float_arg_slots(self.builder.function().all_vns(), |v| self.container_of(v))
                .into_iter()
