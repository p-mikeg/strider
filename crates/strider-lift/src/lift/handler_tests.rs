...
/// No SLEIGH production emits `SegmentOp` or `Indirect`; the decompiler mints
/// both, `SegmentOp` with a host `AddrSpace*` in input 0.
#[test]
fn lift_decompiler_internal_opcodes_fail_closed() {
    for opcode in [Opcode::SegmentOp, Opcode::Indirect] {
        with_test_lifter(|d, rid| {
            let insn = Insn {
                opcode,
                output: Some(reg(0)),
            ...
            let err = d
                .process_insn(rid, &insn, test_addr(), &super::RegionMap::default())
                .expect_err("decompiler-internal opcode must be rejected");
            assert!(
                format!("{err:#}").contains("decompiler-internal"),
                "error must name the contract, got: {err:#}"
            );
        });
    }
...
/// `strider-ir`'s test-only `FunctionBuilder::build_call_other_abi` is a
/// hand-kept copy of this crate's `build_abi_call_other`, and 24 call sites
/// across three crates keep it: nothing but this holds the two to one shape.
///
/// Compares the graph AND the post-call variable bindings, since the writeback
/// the two agree to skip on a terminating op mints no node.
#[test]
fn call_other_abi_matches_the_ir_test_builder() {
    use strider_target::BuiltCallOtherAbi;

    let vns = vec![reg(0), reg(4), reg(8)];
    let ram = Vn {
        size: 4,
        addr_off: 0x2000,
        addr_space: VnSpace::RAM,
    };
    let cases: Vec<(&str, BuiltCallOtherAbi, Option<Vn>, bool)> = vec![
        (
            "register result plus a clobber",
            BuiltCallOtherAbi {
                implicit_reads: vec![reg(4)],
                implicit_writes: vec![reg(8)],
        ...
        (
            "ram result under a memory clobber, whose slot both drop",
            BuiltCallOtherAbi {
                implicit_reads: vec![],
                implicit_writes: vec![reg(8)],
        ...
        (
            "terminating op, whose writeback both skip",
            BuiltCallOtherAbi {
                implicit_reads: vec![reg(4)],
                implicit_writes: vec![reg(8)],
                clobbers_memory: false,
    ...
    /// Every node, plus what each tracked variable now reads back as.
    fn state(d: &FunctionLifter<'_, TestReader>, vns: &[Vn]) -> (Vec<String>, Vec<String>) {
        let f = d.builder.function();
        let nodes = f
            .graph()
            .all_node_ids()
            .map(|id| {
                format!(
                    "{:?} in={:?} out={:?} name={:?}",
                    f.node_kind(id),
                    f.node_inputs(id).iter().collect::<Vec<_>>(),
                    f.node_outputs(id),
                    f.side_tables().call_other_name(id),
                )
            })
            .collect();
        let vars = vns
            .iter()
            .map(|vn| {
                let read = d.builder.read_variable(vn).map_err(|e| e.to_string());
                format!("{vn:?}={read:?}")
            })
            .collect();
        (nodes, vars)
    ...
    for (label, abi, output, terminate) in cases {
        let mut lifted = None;
        with_test_lifter_tracking(vns.clone(), |d, _| {
            let arg = d
                .builder
                .build_int_const(5u128, ValueType::I32)
                .expect("const");
            d.build_abi_call_other(7, "syscall", &[arg], &abi, output, terminate)
                .expect("lift path");
            lifted = Some(state(d, &vns));
        });
        let mut built = None;
        with_test_lifter_tracking(vns.clone(), |d, _| {
            let arg = d
                ...
                .expect("const");
            d.builder
                .build_call_other_abi(7, "syscall", &[arg], &abi, output, terminate)
                .expect("builder path");
            built = Some(state(d, &vns));
        });
        assert_eq!(lifted, built, "{label}");
    }
}
