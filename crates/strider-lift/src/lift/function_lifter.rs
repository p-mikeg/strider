...
impl<'a, R: rsleigh::MemReader> FunctionLifter<'a, R> {
    ...
    pub(crate) fn new(
        ...
        // `FunctionBuilder::new` seeds the function's OWN convention only, so an
        // override's float argument registers would be untracked and
        // `float_arg_slots` would gap on them, truncating the call's float
        // arguments at the gap.
        for v in per_address_ccs
            .values()
            .flat_map(|o| o.arg_passing_regs_float.iter())
        {
            if !all_vns.contains(v) {
                all_vns.push(*v);
            }
        }
