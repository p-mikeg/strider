...
pub(crate) struct FunctionLifter<'a, R: rsleigh::MemReader> {
    ...
    /// The most recent value committed to `ISAModeSwitch` (ARM's `setISAMode`
    /// reads it, MIPS's `JXWritePC` writes it) and the machine address that
    /// committed it: the mode the instruction decodes its branch target(s) in.
    /// The region's terminating `IndirectBranch` takes it as its mode input iff
    /// they share that address.
