...
fn aliased_float_arg_regs_keep_their_abi_positions() {
    ...
    with_test_lifter_cc(cc, vec![q0, d2, sp], |d, _rid| {
        ...
        assert!(
            st.float_arg_index_to_values(3).is_empty(),
            "untracked d3 leaves a gap at its own index"
        );
    });
}

/// An untracked leading float argument register leaves a GAP: the registers
/// after it keep their ABI positions.
#[test]
fn untracked_float_arg_reg_leaves_a_gap() {
    ...
    with_test_lifter_cc(cc, vec![d1, sp], |d, _rid| {
        ...
        let st = d.builder.function().side_tables();
        assert!(
            st.float_arg_index_to_values(0).is_empty(),
            "the function never names d0, so float param 0 has no carrier"
        );
        assert_eq!(
...
fn arm_neon_float_args_keep_their_abi_positions_end_to_end() {
    ...
    // [Control, Memory, target, r0..r3, d0..d3].
...
/// d4, so float positions 4 and 6 have no tracked carrier and d5 / d7 are
/// scratch rather than arguments 4 and 5.
#[test]
fn float_call_args_keep_their_abi_positions() {
    ...
    assert!(
        slots[4].is_none() && slots[6].is_none(),
        "the fixture must leave float positions 4 and 6 untracked, got {slots:?}"
    );
    ...
    assert_eq!(
        float_args.len(),
        leading,
        "float argument {leading} has no carrier, so nothing past it is an \
         argument; got {} float arguments",
        float_args.len(),
    );
    for (j, arg) in float_args.iter().enumerate() {
        let reg = slots[j].expect("a leading slot is tracked");
        assert_eq!(
            f.initial_var_value(&reg),
            Some(*arg),
