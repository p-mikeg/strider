...
pub(crate) fn try_match(
    ...
    let satisfied = Cell::new(0);
    let ctx = Ctx {
        matcher,
        pat,
        satisfied: &satisfied,
    };
    let mut counted = counting(&satisfied, k);
    try_match_at(
        &ctx,
        root,
        root_node,
...
fn try_match_at(
    ...
    if nd.alternation {
        ...
        for alt in inputs {
            ...
            let accepted = try_match_at(
                ...
                &mut |b| {
                    finish_node(ctx, nd, pat_node, ir_node, root_value, out_vertex, b)
                        && continue_node(ctx, nd, ir_node, b, k)
                },
            );
            if accepted {
                return true;
    ...
    let mark = bindings.mark();
    {
        ...
        let mut finalize = |b: &mut Bindings| -> bool {
            ...
            if finish_node(ctx, nd, pat_node, ir_node, root_value, out_vertex, b)
                && continue_node(ctx, nd, ir_node, b, k)
            {
                return true;
            }
...
/// configuration, and left to the compiler's judgement the extra call costs
/// ~8% on `benches/matcher.rs`'s `matcher_find_all_add_const`.
#[inline(always)]
fn continue_node(
    ctx: &Ctx,
    nd: &PatNode,
