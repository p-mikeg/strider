...
pub enum TemplateTy {
    ...
    /// instantiation time from the value the capture matched.  Unlike
    /// [`InheritInput`] (which is root-relative), this is binding-relative:
    /// use it when a materialised interior node's width comes from a captured
    /// operand that the rewrite root's shape does not expose — e.g. the
    /// `And(x, mask)` / mask in `Sless(x<<C, 0) → Xor(Equal(And(x,mask),0),1)`,
    /// whose `I1` `Xor` root has no `x`-wide input.
    ...
    for vtx in order {
        ...
        let kind = match &nd.kind {
            ...
            TmplNodeKind::Build(TemplateKind::Fn(f)) => {
                ...
                let value_ty = node_value_ty(template, vtx, root_ty, root_in_ty, &binding_tys);
                let ctx = TemplateCtx {
        ...
        let outputs = output_kinds_for(template, vtx, root_ty, root_in_ty, &binding_tys);
...
/// (`root_ty`), its first-input type (`root_in_ty`), and the per-capture width
/// map (`binding_tys`).  An `InheritBinding` whose capture didn't resolve to a
/// typed value falls back to `root_ty`.
fn resolve_ty(
    ty: TemplateTy,
    root_ty: ValueType,
    root_in_ty: ValueType,
...
fn resolved_output_kind(o: &TmplOutput, root_ty: ValueType, root_in_ty: ValueType, binding_tys: &FxHashMap<crate::Capture, ValueType>) -> ValueKind {
    match o.kind {
        ...
        OutputKindSpec::Value(_) | OutputKindSpec::AnyValue | OutputKindSpec::Any => {
            ValueKind::Typed(resolve_ty(o.ty, root_ty, root_in_ty, binding_tys))
        }
    }
}
...
fn node_value_ty(template: &Template, node_vtx: NodeId, root_ty: ValueType, root_in_ty: ValueType, binding_tys: &FxHashMap<crate::Capture, ValueType>) -> ValueType {
    template
        .graph
        ...
        .find_map(|out_vtx| {
            ...
            match resolved_output_kind(o, root_ty, root_in_ty, binding_tys) {
                ValueKind::Typed(t) => Some(t),
                _ => None,
            }
        })
...
fn output_kinds_for(template: &Template, node_vtx: NodeId, root_ty: ValueType, root_in_ty: ValueType, binding_tys: &FxHashMap<crate::Capture, ValueType>) -> Vec<ValueKind> {
    ...
    for out_vtx in template.graph.produced_outputs(node_vtx).iter().copied() {
        if let TmplValue::TmplOutput(o) = template.graph.output_weight(out_vtx) {
            by_slot.insert(o.slot, resolved_output_kind(o, root_ty, root_in_ty, binding_tys));
        }
    }
    if by_slot.is_empty() {
