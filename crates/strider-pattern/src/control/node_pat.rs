...
//! `Load` / `Store` / `Phi` / `MemPhi` / `ValuePhi`) lowers to the same
...
pub(crate) enum AnchorKind {
    /// A value output at the given slot (`Load` / `Phi` / `ValuePhi`).
    Value(usize),
...
pub(crate) enum RootKind {
    ...
    /// also nest as value operands — `Load` / `ValuePhi`).
    Value,
...
impl LowerResult {
    ...
    pub(crate) fn mem_out(self) -> PatOutRef {
...
impl NodePat {
    ...
    pub(crate) fn with_mem_out(self, slot: usize) -> Self {
    ...
    #[allow(clippy::expect_used)]
    pub(crate) fn compile_value(self, b: &mut MatcherBuilder) -> PatOutRef {
