...
//! so order carries no meaning and two overlapping arms both fire, giving one
//! match each. [`OneOf::first`] is the ordered variant, cutting to the first
//! arm that yields a match. Captures under an arm that did not fire stay
//! unbound, so `Match::value` / `node` returning `None` tells which arm won.
...
impl OneOf {
    ...
    /// graph. [`match_at`](crate::Matcher::match_at) and
    /// `find_unique` stop at the first match; [`OneOf::first`] cuts instead of
    /// enumerating.
    ...
    /// An ordered choice: cut to the first alternative that yields a match.
    /// The cut is on a produced match, so an arm a guard above rejects falls
