...
/// True when `stored`, an `out_ty`-wide constant, is `v` held at one of the
/// candidate widths 8/16/32/64/128 and widened to `out_ty` by zero extension
/// or by sign extension. `v` stored at `out_ty` itself is the widest case.
