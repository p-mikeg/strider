...
/// Every integer width the IR can hold a constant at, ascending. A candidate
/// width narrower than the constant's own is a width it could have been widened
/// FROM; the constant's own width is the exact case.
const INT_WIDTHS: [usize; 15] = [1, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 96, 112, 128, 256];

/// True when `stored`, an `out_ty`-wide constant, is `v` held at one of
/// [`INT_WIDTHS`] and widened to `out_ty` by zero extension or by sign
/// extension. `v` stored at `out_ty` itself is the widest case, so every
/// `ValueType` width is a candidate rather than only the powers of two: an
/// `I40` constant is not reachable through an `I32` or `I64` probe.
