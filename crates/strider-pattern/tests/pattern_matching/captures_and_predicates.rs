...
fn var_used_three_times_enforces_all() {
    ...
    assert_eq!(m.get_uint(x, function.graph()), Some(7));
}
...
fn get_int_const_on_non_const_returns_none() {
    ...
    assert_eq!(m.get_uint(x, function.graph()), None);
}
...
fn get_bool_const_and_float_bits_helpers() {
    ...
    assert_eq!(m.get_bool(v, function.graph()), Some(true));
    // Not a float.
    assert_eq!(m.get_float_bits(v, function.graph()), None);
}
