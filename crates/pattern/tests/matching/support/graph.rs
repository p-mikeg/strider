...
impl Tb {
    ...
    pub fn raw(
        ...
        let mut me = Self::bare(vars, arg_passing, callee_saved, ret_regs, sp, ret_stack_pop);
        let region = me.region();
        me.fb.set_entry_region(region).expect("set_entry_region");
        me.enter(region);
        me
    }
    ...
    pub fn bare(
        ...
        let fb = FunctionBuilder::new_raw(
            vars,
            arg_passing,
            callee_saved,
            ret_regs,
    ...
    pub fn fun(
        ...
        self.fb.build_float_unary_op(v, op, ty).expect("float_unary_op")
    }
