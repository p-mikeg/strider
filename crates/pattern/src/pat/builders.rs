...
//! `FloatBinaryOpPat`), the four memory builders (`LoadPat`, `StorePat`,
//! `StackStorePat`, `StackStorePhiPat`), and the two phi/function-arg
//! builders (`PhiPat`, `FunctionArgPat`) are hand-rolled so they can emit
//! trait-backed [`NodePat`]s directly.  The remaining builders for the
//! control family (Call / CallOther / Return / If) are still generated by
//! the [`define_pat_builder!`] macro on the Legacy `PatKind` dispatch path.
