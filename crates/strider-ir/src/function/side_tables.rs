...
/// Interner key for a distinct `(base, offset)` decomposition, stack or heap.
...
impl SideTables {
    ...
    /// Links `src`'s fingerprint under `dst`'s. A live alias, not a copy:
    /// contributors added to `src` afterwards are `dst`'s too.
