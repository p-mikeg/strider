...
/// Builders do not auto-coerce, so the caller widens the narrow operand
/// itself and the build then takes it.
#[test]
fn build_int_binary_op_takes_an_explicitly_widened_operand() -> Result<()> {
    let mut b = empty_builder()?;
...
#[test]
fn build_float_const_masks_bits_above_the_declared_width() -> Result<()> {
    let mut b = empty_builder()?;
    let bits = 1.0f32.to_bits() as u64;
    let clean = b.build_float_const(bits, ValueType::F32);
    let garbage = b.build_float_const(bits | 0xDEAD_BEEF_0000_0000, ValueType::F32);
    assert_eq!(producer_kind(&b, clean), NodeKind::FloatConst(bits));
    assert_eq!(
        clean, garbage,
        "an F32 pattern with garbage upper bits must dedup onto the masked node"
    );
    Ok(())
}
...
/// A 9-byte varnode is `is_wide_int`, yet its constants still fit the `u128`
/// carrier, so they intern as `Bits`.
#[test]
fn i72_const_round_trips_through_the_interner() -> Result<()> {
    ...
    // Masking to the declared width drops the carry out of bit 71.
...
fn build_call_other_without_result_advances_ctrl_only() -> Result<()> {
    ...
    let (node, result) = b.build_call_other(7, &[], &[], false, false)?;
    assert!(result.is_empty(), "no output vn -> no ret-val output");
...
fn build_call_other_with_result_returns_typed_value() -> Result<()> {
    ...
    let (node, outputs) = b.build_call_other(3, &[arg], &[out_vn], false, false)?;
    let val = outputs
        .first()
        .copied()
        .ok_or_else(|| anyhow!("output vn = Some -> ret-val output"))?;
    assert_eq!(
...
fn memory_output_of_finds_call_other_memory_slot() -> Result<()> {
    ...
    let mut b = builder_with_region_tracking(vec![out_vn])?;
    let (node, _) = b.build_call_other(4, &[], &[out_vn], false, false)?;
...
fn build_call_other_rejects_non_value_arg() -> Result<()> {
    ...
    let res = b.build_call_other(0, &[mem], &[], false, false);
...
/// The whole slot round trip: leading reads become inputs, trailing output vns
/// become clobber outputs written back to their registers.
#[test]
fn build_call_other_lays_out_reads_result_and_clobbers() -> Result<()> {
...
#[test]
fn clobber_writeback_rejects_an_untracked_register() -> Result<()> {
    let mut b = builder_with_region()?;
    // No tracked variables, so this has no enclosing container.
    let untracked = reg_vn(0, 4);
    let res = b
        .build_call_other(11, &[], &[untracked], false, false)
        .and_then(|(_, outputs)| b.write_variable(&untracked, outputs[0]));
    assert!(res.is_err(), "untracked clobber register must error");
    Ok(())
}
...
fn build_call_other_no_args_emits_ctrl_mem_only() -> Result<()> {
    let mut b = builder_with_region()?;
    let (node, result) = b.build_call_other(0, &[], &[], false, true)?;
    assert!(result.is_empty(), "no output vn -> no ret-val output");
...
fn build_call_other_terminate_true_closes_region() -> Result<()> {
    let mut b = builder_with_region()?;
    b.build_call_other(0, &[], &[], false, true)?;
    let ctrl = b.cur_region_control();
    assert!(
        ctrl.is_err(),
...
fn build_call_other_terminating_and_memory_clobbering_closes_region() -> Result<()> {
    let mut b = builder_with_region()?;
    b.build_call_other(0, &[], &[], true, true)?;
    let ctrl = b.cur_region_control();
    assert!(
        ctrl.is_err(),
...
fn build_call_other_terminate_false_keeps_region_open() -> Result<()> {
    let mut b = builder_with_region()?;
    b.build_call_other(0, &[], &[], false, false)?;
    let ctrl = b.cur_region_control();
    assert!(
        ctrl.is_ok(),
...
/// A `FloatConst` wider than the `u64` payload, which `build_float_const`
/// refuses. Built directly so the fold guards below still see the shape they
/// exist for.
fn unrepresentable_float_const(
    b: &mut impl crate::IRBuilder,
    bits: u64,
    ty: ValueType,
) -> crate::node::ValueId {
    let node = b.create_node(
        NodeKind::FloatConst(bits),
        [],
        [crate::node::ValueKind::Typed(ty)],
    );
    b.function().node_outputs(node)[0]
}

#[test]
fn float_bits_to_int_f80_emits_node_not_const() -> Result<()> {
    let mut b = empty_builder()?;
    let float_const = unrepresentable_float_const(&mut b, 0xBEEFu64, ValueType::F80);
...
fn bits_casts_fold_by_payload_width() -> Result<()> {
    ...
    let f128_const = unrepresentable_float_const(&mut b, 0xBEEF, ValueType::F128);
...
#[test]
fn raw_builder_filters_contained_unique_varnodes() -> Result<()> {
    let cases: [(&str, rsleigh::Vn, rsleigh::Vn); 2] = [
        (
            "raw_builder_filters_overlapping_unique_varnodes",
            unique_vn(0x100, 8),
            unique_vn(0x100, 4), // same offset, narrower
        ),
        (
            "raw_builder_filters_mid_offset_unique_subvarnode",
            unique_vn(0x200, 8),
...
#[test]
fn raw_builder_keeps_disjoint_unique_varnodes() -> Result<()> {
