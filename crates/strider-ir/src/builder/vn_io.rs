//! Register-aliasing read/write helpers on [`FunctionBuilder`].
//!
//! Overlapping sub-registers (`al`/`ah`/`ax`/`eax`/`rax` on x86-64, low-byte
//! slices of x87 ST registers, the s/d/q SIMD views on aarch64, etc.) are
//! handled by always reading and writing through the largest containing
//! tracked register and inserting bit-shift / mask operations for
//! sub-register slices.
//!
//! These methods translate a [`rsleigh::Vn`] in REGISTER / UNIQUE space into
//! the IR primitives a sub-register read or write needs.  The endianness used
//! by the bit-shift formula comes from the builder's [`Function`]
//! ([`crate::Function::endianness`]).

use anyhow::{anyhow, bail};

use crate::error::Result;
use crate::node::ValueType;
use crate::ops::{ExtendOp, IntBinaryOp};
use crate::Value;
...
impl FunctionBuilder {
    ...
    pub(crate) fn calculate_reg_shift_from_container(
        ...
        match self.function.endianness() {
    ...
    pub fn read_reg_vn(&mut self, reg: &rsleigh::Vn) -> Result<Value> {
        let ctx = match self.enter_sub_register(reg, "read_reg_vn")? {
            SubRegOutcome::Direct { container_reg } => {
                ...
                return self.read_variable(&container_reg);
            }
    ...
    /// container, has an unsupported byte size, or an underlying builder
    /// node-construction call fails.
    pub fn write_reg_vn(&mut self, reg: &rsleigh::Vn, val: Value) -> Result<()> {
...
mod shift_formula_tests {
    ...
    // unit-testable without spinning up a full FunctionBuilder.
