//! Value-only DAG generator for the workspace's property tests: a sequence of
//! [`FunctionBuilder`] actions replayed against type-tagged operand buckets,
//! mirroring `cranelift-fuzzgen`. Control-flow properties stay in hand-authored
//! fixtures.

use proptest::prelude::*;
use strider_ir::node::ValueType;
use strider_ir::{ExtendOp, FunctionBuilder, IRBuilderExt, IntBinaryOp, IntCmpOp, IntUnaryOp};
...
pub fn step_seq() -> impl Strategy<Value = Vec<Step>> {
...
pub fn replay(steps: &[Step]) -> Option<strider_ir::Function> {
    let mut b = crate::empty_builder().ok()?;
    ...
    for s in steps {
        let lift_addr = crate::SENTINEL_LIFT_ADDR.wrapping_add(step_lift_off(s) as u64);
        b.set_lift_addr(Some(lift_addr));
        apply_step(&mut b, &mut pools, s);
    }
    b.set_lift_addr(Some(crate::SENTINEL_LIFT_ADDR));
...
fn apply_step(b: &mut FunctionBuilder, pools: &mut Pools, s: &Step) {
    match s {
        ...
        Step::EmitTruncate {
            ...
            // Both builders NO-OP when the source already fits, returning the
            // input: pushing that into the destination bucket poisons the pool
            // with a value of the wrong width, which every later step that
            // picks it then rejects.
...
mod tests {
    ...
    /// A widening `EmitTruncate` no-ops, so the builder hands back the I32
    /// input. Pushed into the I64 bucket it displaces nothing but shifts every
    /// later pick, and the step that lands on it fails its width check and
    /// emits nothing.
    #[test]
    fn a_no_op_width_change_does_not_poison_the_destination_bucket() {
        let f = replay(&[
            Step::EmitIntConst {
                width: ValueType::I64,
                value: 1,
            ...
            Step::EmitTruncate {
                src_width: ValueType::I32,
                dst_width: ValueType::I64,
                src_idx: 0,
                lift_off: 2,
            },
            // Index 1 of the I64 bucket: the poisoned entry, if it got in.
            Step::EmitBinaryOp {
                width: ValueType::I64,
                op: IntBinaryOp::Add,
                lhs_idx: 1,
        ...
        .expect("the sequence returns a value");
        assert!(
            f.walk()
                .any(|n| matches!(f.node_kind(n), NodeKind::IntBinaryOp(_))),
            "the Add was skipped: an I32 value reached the I64 bucket",
        );
    }
}
