...
5. Add an in-place edit if appropriate. The apply functions are **defined** in `crates/opt/src/indirect_branch_resolve/inplace.rs` and **re-exported** from `crates/strider/src/indirect_resolve/inplace.rs` for the orchestrator's bridge layer.  Variants: `LinkRegister` -> `apply_link_register` (append ABI ret-val regs to placeholder Return); `Single` (tail call) -> `apply_tail_call` (rewrite as `Call` + `Return` chain at the tail); `Multiple` (jump table) -> leave for orchestrator CFG rebuild via `ResolvedTargets` propagation.
