...
2. **Read the existing 9 rules in `build_rules` (around `mod.rs:310`)** to understand the rule numbering and shape vocabulary. Rules 1-7 are AArch64-style (carry bit / sign / overflow trees); rules 8-9 are Thumb cast-to-int unaries.
...
7. **Register the rule in `build_rules`.** Append to the `vec!` returned by `build_rules` (`mod.rs:310`). Maintain numerical comments so downstream readers can cross-reference rule N to its semantics.
