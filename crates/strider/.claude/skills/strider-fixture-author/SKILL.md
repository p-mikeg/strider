...
- **Skipping per-arch ignores when the shape only exists on some arches.** The macro expands to 15 tests; an unconditional assertion will fail noisily on every arch where the source compiles to a different idiom. Tie ignore reasons to entries in `docs/superpowers/plans/2026-04-25-analyzer-known-issues.md`.
