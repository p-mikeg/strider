...
1. Pick the root builder by IR root kind. Use `CallPat` (`crates/pattern/src/pat/builders/call.rs`) for call sites with `.at(addr)`, `.at_any([...])`, `.target(p)`, `.arg(idx, p)`. Use `RetPat` (`crates/pattern/src/pat/builders/ret.rs`) for returns; `LoadPat` / `StorePat` (`crates/pattern/src/pat/builders/memory.rs`) for generic memory; `StackStorePat` / `StackStorePhiPat` only after `StackStoreDetect` has run; `IfPat` (`crates/pattern/src/pat/builders/branch.rs`) for branches (canonical layout only); `PhiPat` (`crates/pattern/src/pat/builders/phi.rs`) / `phi_for(vn)` for `VarPhi` only. Leaf builders: `int_const(n)`, `signed_int_const(n)`, `int_const_any_of([...])`, `var(c)`, `any()`, `predicate(f)`.
...
3. Bind captures via `Capture` (`crates/pattern/src/var.rs`); a Capture reused across patterns means the same `(NodeId, NodeOutputId)` must bind everywhere it appears. In Python, prefer the str-keyed form (`add("x", "x")`) — strings intern globally via `pattern.rs::intern_capture`.
4. Choose a matcher entry point on `Matcher<'g>` (`crates/pattern/src/matcher/mod.rs`). Single pattern: `find_all(&pat)`. N patterns over one preorder walk, no shared captures: `find_all_multi(&[&p1, &p2, ...])`. N patterns with shared captures (cross-pattern join): `find_all_requirements(&[...])` — applies the cross-product filter so shared captures bind identically across the whole tuple.
