...
3. Bind captures via `Capture` (`crates/pattern/src/var.rs`); a Capture reused across patterns means the same `(NodeId, NodeOutputId)` must bind everywhere it appears. In Python, prefer the str-keyed form (`add("x", "x")`) — strings intern globally via `pattern.rs::intern_str`.
