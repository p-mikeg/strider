...
1. Decide the trait. `Optimizer` for graph-level passes; `OptimizerOnBuilt` if you operate on a `BuiltFunctionGraph`. Look at `crates/opt/src/if_cond_inversion/` (`OptimizerOnBuilt`) and `crates/opt/src/constant_fold/` (`Optimizer`) for templates.
...
6. Propagate asm-fingerprints (REQUIRED). Every newly-created `NodeId` that replaces or derives from an existing node must inherit contributors via `Graph::extend_asm_fingerprint_from(new, contributor)` (`crates/ir/src/graph/store.rs:160`). The contract is superset-only: never shrink, never replace with a node whose fingerprint is a strict subset of an ancestor's. If the rewrite has multiple contributors, call `extend_asm_fingerprint_from` once per contributor. Canonical example: `crates/opt/src/flag_cmp_canonicalize/mod.rs`.
