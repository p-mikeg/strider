...
1. Decide the trait. **Prefer `Optimizer`** — its `optimize(&self, ctx: &mut pattern::RewriteCtx<'_>)` signature is the higher-level surface (`RewriteCtx` Derefs to `Graph` and exposes `preorder()` / `preorder_kind()` ergonomically), and the blanket `impl<T: Optimizer> OptimizerRaw for T` adapts via `with_rewrite_ctx` so it slots into the same pipeline as raw `OptimizerRaw` impls. Implement `OptimizerRaw` directly only when you need the lower-level `(&mut Graph, NodeId)` pair (e.g. external bindings that hold type-erased `Box<dyn OptimizerRaw>` adapters). Templates: `crates/opt/src/if_cond_inversion/` and `crates/opt/src/constant_fold/` are both `Optimizer`.
