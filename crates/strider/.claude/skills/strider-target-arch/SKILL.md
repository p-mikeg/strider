...
2. Add a constructor in `crates/target/src/arch.rs::SleighArch` (e.g. `SleighArch::riscv64()`). Wire `sla_spec`, `pspec`, `endianness`, `preset`. Existing presets to mirror: `x86_64`, `x86`, `aarch64`, `aarch64be`, `arm`, `arm_be`, `arm_thumb`, `mipsbe32`, `mipsle32`, `mipsbe64`, `mipsle64`, `ppc32be`, `ppc32le`, `ppc64be`, `ppc64le`.
3. Add a CC preset in `crates/target/src/calling_convention/mod.rs`. Locate by symbol name rather than line number — the preset table shifts as new presets are added.  Use `grep -n "pub fn x86_64_systemv\|pub fn aarch64_aapcs64\|pub fn arm_aapcs\|pub fn mips_o32\|pub fn mips_n64\|pub fn x86_cdecl" crates/target/src/calling_convention/mod.rs` to find the current entry point of each preset and mirror the closest one.  Fill: stack-pointer reg name; integer + float arg regs (positional); integer + float return-value regs; callee-saved regs; `stack_arg_offsets` (positional offsets for stack-passed args); `ret_stack_pop` (`0` for callee-cleanup AAPCS, non-zero for caller-cleanup cdecl which pops 4/8); link-register varnode name (`Some("ra")`, `Some("lr")`, or `None`).
...
6. Register-aliasing. If the arch has overlapping registers strider doesn't yet handle (something other than the documented widths 1, 2, 4, 8, 10, 16, 32, 64 bytes — widths > 16 use a degraded `u128::MAX` mask), extend `crates/pcode-lift/src/vn_io.rs::vn_mask` and `find_largest_fitting_register`.
