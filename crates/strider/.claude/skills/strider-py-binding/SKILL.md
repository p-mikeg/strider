...
4. Apply Python-side ergonomics where they help. String-keyed capture interning (`pattern.rs::intern_str`) lets users write `add("x", "x")` instead of allocating a `Capture` object. Subclassable Python ABCs are used for callback-style readers (`MemReader`, `ReadOnlyMemory`).
