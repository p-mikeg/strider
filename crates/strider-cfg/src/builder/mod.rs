...
pub struct Builder<'a, R: rsleigh::MemReader> {
    ...
    /// [`Self::find_region_containing_addr`]'s reverse walk.  Zero while
    /// instruction streams do not overlap.  Monotone: a split only shortens
    /// both halves.
...
impl<'a, R: rsleigh::MemReader> Builder<'a, R> {
    ...
    pub(super) fn add_region(&mut self, region: Region) -> Result<NodeIndex> {
        ...
        let shadowed_span = self
            .find_region_containing_addr(start_addr)
            .map(|(_, shadowed)| shadowed.span_len());
        if let Some(shadowed_span) = shadowed_span {
    ...
    /// `max_shadowed_span` bytes below `addr`, which bounds the walk.
    pub(super) fn find_region_containing_addr(
        ...
        self.start_addr_to_region_id
            ...
            .rev()
            .find_map(|(_, &region_id)| self.region_owning(region_id, addr))
    }
    ...
    pub(super) fn addr_is_interior_non_boundary(&self, addr: PcodeInsnAddr) -> bool {
        self.find_region_containing_addr(addr)
            .is_some_and(|(_, region)| {
                region.start_addr != addr && !region.insns.iter().any(|i| i.addr == addr)
            })
    }
    ...
    /// arm edge was ever wired. Every other terminator edges to the region that
    /// OWNS the address, which `Cfg::region_if` still matches by containment, so
    /// a conditional keeps both arms.
    fn seat_non_boundary_target(
        ...
        let super::types::RegionTerminator::Switch {
            ...
            self.region_graph.add_edge(parent, owner, ());
            return Ok(());
        };
    ...
    fn explore(
        ...
        if let Some((region_id, region)) = self.find_region_containing_addr(addr) {
            ...
            // the last-enqueued edge.
            let is_start = region.start_addr == addr;
            let is_boundary = region.insns.iter().any(|i| i.addr == addr);
    ...
    pub fn build(mut self) -> Result<Cfg> {
        ...
        {
            self.explore(parent_region, address, carried)?;
        }
...
mod tests {
    ...
    fn sleigh_over(bytes: Vec<u8>, base: u64) -> rsleigh::Sleigh<TestReader> {
    ...
    fn branch_indirect_index(bytes: &[u8], base: u64, at: u64) -> u64 {
        let mut sleigh = sleigh_over(bytes.to_vec(), base);
    ...
    fn build_cfg(
        ...
        let arch = strider_target::SleighArch::x86_64();
        let mut sleigh = sleigh_over(bytes, base);
    ...
    fn assert_every_switch_target_has_an_arm(cfg: &crate::Cfg) {
        for rid in cfg.region_ids() {
            ...
            for t in targets {
                assert!(
                    cfg.switch_arm_region(rid, t.addr.into()).is_some(),
