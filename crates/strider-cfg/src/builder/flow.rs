...
impl FlowVars {
    ...
    /// `(10,13)`, `cond_shft` `(9,13)`, ...) flow in their own right; `condit`,
    /// the `(5,13)` field spanning them, is declared `noflow` and is never in
    /// the set. An `IT` does `globalset(inst_next, condit)`, committing all nine
