...
impl Cfg {
    ...
    /// The successor of `switch_region` that starts at `target`: the arm the CFG
    /// builder wired for that jump-table target.  Resolving by successor edge
    /// rather than a global start-address lookup stays correct across a later
    /// `split_region` that re-targets the incoming edge.  Matches on the full
    /// `start_addr`: a target landing inside an already-decoded region splits
    /// it, so a wired arm always begins exactly at the target, and a target is
    /// always a machine-instruction start.  A successor beginning MID-pcode at
    /// the same machine address (a `CondBranch` into a pcode sequence) is a
    /// different region and does not answer for it.
    pub fn switch_arm_region(
        &self,
        switch_region: RegionId,
        target: super::types::MachineInsnAddr,
    ) -> Option<RegionId> {
        let target = super::types::PcodeInsnAddr::at_machine_start(target.addr);
        self.region_graph.neighbors(switch_region).find(|&s| {
            self.region_graph
                .node_weight(s)
                .is_some_and(|region| region.start_addr == target)
        })
    }
}
...
mod tests {
    ...
    use crate::types::{MachineInsnAddr, PcodeInsnAddr, Region};
    ...
    #[test]
    fn switch_arm_region_resolves_each_arm_by_successor_edge() {
        // Three switch arms; each `target` resolves to its own successor, and a
        // target no arm starts at returns None. Resolution is by outgoing edge,
        // so only successors of `src` are considered.
        let mut graph: StableDiGraph<Region, ()> = StableDiGraph::new();
        ...
        assert_eq!(
            cfg.switch_arm_region(src, MachineInsnAddr { addr: 0x3000 }),
            Some(arm_b)
        );
        assert_eq!(
            cfg.switch_arm_region(src, MachineInsnAddr { addr: 0x2000 }),
            Some(arm_a)
        );
        assert_eq!(
            cfg.switch_arm_region(src, MachineInsnAddr { addr: 0x5000 }),
            None,
    ...
    #[test]
    fn switch_arm_region_does_not_match_a_mid_pcode_successor() {
        let mut graph: StableDiGraph<Region, ()> = StableDiGraph::new();
        let src = graph.add_node(make_region(&[(0x1000, 0)]));
