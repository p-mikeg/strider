...
for a CallOther classified as a `BUG()`-class trap, `TailCall` carrying the
callee, `Switch` carrying a jump table's arms and the dispatch address, and
`UnresolvedIndirectBranch`. Only `Unconditional`, `CondBranch` and `Switch`
...
stream the branch jumps to; off a `Switch` the arm is dropped instead, and a
`Switch` losing its last arm degrades to `UnresolvedIndirectBranch`.
...
- `undecodable_seeded_targets`: a seeded target that would not decode. A direct
  edge that fails is an `Err` on the whole build, a seeded one is a
  misclassification, so its edge is dropped and its address reported.
...
- `interior_branch_targets`: the off-boundary targets above, whose edge is not
  exact or absent entirely.
