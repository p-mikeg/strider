...
/// Structural equality by re-reading the candidate out of the store. Generic
/// counterpart to [`hash_key`].
...
fn canonicalize_merges_a_mutated_twin() {
    // Rewiring C = Add(x, z) to Add(x, y) makes it a twin of the cached A.
    ...
    // Operand order differs, so this shape is unique.
...
fn canonicalize_reinserts_unique_mutated_node_then_dedups() {
    // A node whose inputs change (so it is evicted) but has NO twin must be
    // re-inserted as its own representative, so a later identical create_node
    // dedups to it.
    let mut g = TestGraph::new();
    let x = const_node(&mut g, 1);
    let y = const_node(&mut g, 2);
    ...
    // Rewire to the unique shape Add(x, z); update_input evicts it.
...
/// Scans every node's inputs, independent of the use-list, so it cross-checks
/// it.
...
/// Bidirectional use-list consistency: every input edge appears in its value's
/// use-list, and every use-list entry corresponds to a real input edge.
...
proptest! {
    ...
    /// Build a random valid DAG, then assert the structural invariants hold.
    #[test]
    fn random_dag_preserves_invariants(ops in prop::collection::vec(op_strategy(), 1..80)) {
            ...
            match op {
                ...
                Op::Replace(a, b) => {
                    ...
                    if old != new {
                        ...
                        // `new` gained exactly old's former uses.
        ...
        // Toposort yields each producer before its consumers.
    ...
    /// retain_reachable keeps exactly the reachable set, and the remap is
...
fn multi_output_node() {
    let mut g = TestGraph::new();
    // Region is non-cacheable, so it can be shaped freely.
...
fn mutating_cached_node_evicts_it() {
    ...
    // Must invalidate the stale (Add, [x, y], _) entry BEFORE the change.
    let slot0 = g.node_input_id_at(add, 0).ok().unwrap();
    g.update_input(slot0, z);
    assert_eq!(g.nth_input(add, 0), Some(z), "input was rewritten");

    // A fresh node over the ORIGINAL inputs proves the stale entry was evicted.
...
fn compaction_rebuilds_cache() {
    ...
    // Keeps the Add reachable by inputs.
...
fn remove_node_inputs_batch_removes_many_in_one_pass() {
    ...
    // Surviving input_index values compacted contiguously.
...
fn remove_node_inputs_batch_empty_and_out_of_bounds() {
    ...
    // Out-of-bounds ignored; duplicates collapse harmlessly.
...
fn remove_node_inputs_batch_matches_repeated_single_removes() {
    // The batch verb must match removing the same slots one at a time.
...
fn replace_all_uses_moves_uses() {
    ...
    let a = const_node(&mut g, 1);
    let b = const_node(&mut g, 2);
    // Regions don't dedup, so these are three distinct consumers of `a`.
...
fn replace_all_uses_self_is_noop_returns_false() {
    // A self-redirect replaces nothing and reports `false`, even with live uses.
    let mut g = TestGraph::new();
    let a = const_node(&mut g, 1);
    let r = region_node(&mut g);
    g.add_node_input(r, a);
...
fn add_self_loop_input_then_canonicalize() {
    // A node consuming its own output must not panic, must find no spurious
    // twin, and must remain its own canonical entry.
    let mut g = TestGraph::new();
    let x = const_node(&mut g, 1);
    let add = g.create_node(TestKind::Add, [x, x], [TestVal::Int]);
    let add_val = g.node_outputs(add)[0];
    // Wire the node's own output back in as a third input: a self-loop.
    ...
    // Dedup proves the re-insert re-established the cache entry.
...
fn dfs_post_order_runs_on_graph() {
    ...
    // Ordering against the produced value is not required here (post-order
    // over outgoing edges), but the root must come last.
...
mod node_cache_hooks {
    ...
    /// Caches every kind, hashes the whole tuple, eq by re-read.
    struct CacheAll;
    ...
    /// `hash` ALWAYS returns `u64::MAX`, the sentinel the cache must remap
    /// internally for eviction and membership to stay correct. Equality still
    /// discriminates by re-read.
    ...
    fn invalidate_then_recreate_allocates_fresh() {
        ...
        // Detaching inputs invalidates `n`'s entry, so re-creating the original
        // key must NOT revive it.
    ...
    fn rebuild_reestablishes_dedup_after_compaction() {
        ...
        // Proves rebuild re-keyed the cache over the renumbered ids.
    ...
    fn sentinel_hash_still_caches_and_evicts() {
        // Every hash is u64::MAX, so the cache's sentinel remap must keep dedup,
        // eviction, and membership correct.
        let mut g: Graph<u8, u8, SentinelHashPolicy> = Graph::new();
        let x = g.create_node(1u8, [], [9u8]);
        let xv = g.node_outputs(x)[0];

        // Both land in one bucket, discriminated only by re-read eq.
        ...
        // Eviction must still find `a`'s bucket despite the remapped hash.
    ...
    fn canonicalize_ignores_hash_collision_non_twin() {
        // When every node lands in one bucket, a mutated node colliding with a
        // structurally DIFFERENT node must not be reported as a twin:
        // canonicalize re-reads structure via eq and re-inserts it as its own
        // representative.
        let mut g: Graph<u8, u8, SentinelHashPolicy> = Graph::new();
        let x = g.create_node(1u8, [], [9u8]);
        let xv = g.node_outputs(x)[0];
...
#[test]
fn out_of_bounds_remove_node_input_leaves_dedup_intact() {
    let mut g = TestGraph::new();
    let x = const_node(&mut g, 1);
    let y = const_node(&mut g, 2);
    let a = g.create_node(TestKind::Add, [x, y], [TestVal::Int]);

    assert!(!g.remove_node_input(a, 7));

    let twin = g.create_node(TestKind::Add, [x, y], [TestVal::Int]);
    assert_eq!(a, twin, "a rejected remove must not evict the dedup entry");
    assert_use_list_consistent(&g);
}
...
fn retain_reachable_rejects_a_duplicate_node_id() {
    ...
    let y = const_node(&mut g, 2);
    let sum = add_node(&mut g, x, y);
    // The const's output is consumed, so a second copy of it has a use-list to
    // corrupt.
