...
impl crate::graph::Graph {
    ...
    fn get_output_type(&self, output: NodeOutputId) -> NodeOutputType {
        self.output_kind(output).as_value().unwrap()
    }
    fn verify_bool_type(&self, outputs: &[NodeOutputId]) {
        for output in outputs {
            assert!(self.output_kind(*output).is_bool());
        }
    }

    fn verify_control_kind(&self, outputs: &[NodeOutputId]) {
        for output in outputs {
            assert!(self.output_kind(*output).is_control());
        }
    }

    fn verify_int_type(&self, outputs: &[NodeOutputId]) {
        for output in outputs {
            assert!(self.output_kind(*output).is_integer());
        }
    }

    fn verify_memory_kind(&self, outputs: &[NodeOutputId]) {
        for output in outputs {
            assert!(self.output_kind(*output).is_memory());
        }
    }

    pub fn node_view(&self, node_id: NodeId) -> NodeView {
        let view = match self.node_kind(node_id) {
            NodeKind::Entry => {
                let [] = self.node_inputs_exact(node_id);
                let [control] = self.node_outputs_exact(node_id);
          
                assert!(self.output_kind(control).is_control());
                NodeView::Entry
            }
