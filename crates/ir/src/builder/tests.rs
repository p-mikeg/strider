...
fn get_unsigned_int_truncates_to_declared_width() -> Result<()> {
    ...
    let out = b.build_int_const(u8::MAX as u64 + 1, NodeOutputType::U8).unwrap();
...
fn get_unsigned_int_is_none_for_non_const() -> Result<()> {
    let mut b = empty_builder()?;
    let lhs = b.build_int_const(1, NodeOutputType::U64).unwrap();
    let rhs = b.build_int_const(2, NodeOutputType::U64).unwrap();
...
fn get_signed_int_sign_extends_negative_u8() -> Result<()> {
    let mut b = empty_builder()?;
    let out = b.build_int_const(u8::MAX as u64, NodeOutputType::U8).unwrap();
...
fn get_signed_int_positive_u8_stays_positive() -> Result<()> {
    let mut b = empty_builder()?;
    let out = b.build_int_const(i8::MAX as u64, NodeOutputType::U8).unwrap();
...
fn truncate_const_folds_to_const() -> Result<()> {
    let mut b = empty_builder()?;
    let out = b.build_int_const(0xABCD, NodeOutputType::U16).unwrap();
...
fn truncate_noop_when_already_narrow_non_const() -> Result<()> {
    ...
    let lhs = b.build_int_const(1, NodeOutputType::U8).unwrap();
    let rhs = b.build_int_const(2, NodeOutputType::U8).unwrap();
...
fn convert_nonzero_int_to_bool_folds_to_true() -> Result<()> {
    let mut b = empty_builder()?;
    let nonzero = b.build_int_const(99, NodeOutputType::U32).unwrap();
...
fn build_int_binary_op_produces_binary_op_node() -> Result<()> {
    let mut b = empty_builder()?;
    let lhs = b.build_int_const(3, NodeOutputType::U64).unwrap();
    let rhs = b.build_int_const(4, NodeOutputType::U64).unwrap();
    let result =
...
fn build_int_cmp_produces_bool_output() -> Result<()> {
    let mut b = empty_builder()?;
    let lhs = b.build_int_const(10, NodeOutputType::U32).unwrap();
    let rhs = b.build_int_const(20, NodeOutputType::U32).unwrap();
...
fn identical_constants_are_deduplicated() -> Result<()> {
    let mut b = empty_builder()?;
    let a = b.build_int_const(77, NodeOutputType::U32).unwrap();
    let c = b.build_int_const(77, NodeOutputType::U32).unwrap();
...
fn different_constants_are_distinct() -> Result<()> {
    let mut b = empty_builder()?;
    let a = b.build_int_const(1, NodeOutputType::U32).unwrap();
    let c = b.build_int_const(2, NodeOutputType::U32).unwrap();
    assert_ne!(a, c);
    Ok(())
}
...
fn get_as_float_bits_returns_none_for_int_const() -> Result<()> {
    let mut b = empty_builder()?;
    let out = b.build_int_const(42, NodeOutputType::U64).unwrap();
...
fn int_bits_to_float_folds_int_const_immediately() -> Result<()> {
    let mut b = empty_builder()?;
    let bits = 1.0f32.to_bits() as u64;
    let int_out = b.build_int_const(bits, NodeOutputType::U32).unwrap();
...
fn build_int_bits_to_float_inserts_node_for_non_const() -> Result<()> {
    let mut b = empty_builder()?;
    let int_val = b.build_int_const(0x3F800000, NodeOutputType::U32).unwrap();
    let zero = b.build_int_const(0, NodeOutputType::U32).unwrap();
...
fn build_cast_to_float_creates_cast_node() -> Result<()> {
    let mut b = empty_builder()?;
    let int_val = b.build_int_const(42, NodeOutputType::U64).unwrap();
...
fn build_float_binary_op_with_int_inputs_auto_casts() -> Result<()> {
    let mut b = empty_builder()?;
    let i1 = b.build_int_const(0x3F800000u64, NodeOutputType::U32).unwrap();
    let i2 = b.build_int_const(0x40000000u64, NodeOutputType::U32).unwrap();
...
fn build_call_other_with_output_returns_typed_value() -> Result<()> {
    let mut b = builder_with_region()?;
    let arg = b.build_int_const(0x42, NodeOutputType::U64).unwrap();
    let out = b
...
fn build_segment_op_produces_pure_node() -> Result<()> {
    let mut b = builder_with_region()?;
    let seg = b.build_int_const(0x10, NodeOutputType::U16).unwrap();
    let off = b.build_int_const(0x100, NodeOutputType::U32).unwrap();
...
fn build_cpool_ref_produces_typed_node() -> Result<()> {
    let mut b = builder_with_region()?;
    let r0 = b.build_int_const(0xAA, NodeOutputType::U32).unwrap();
    let r1 = b.build_int_const(0xBB, NodeOutputType::U32).unwrap();
...
fn build_new_produces_typed_node() -> Result<()> {
    let mut b = builder_with_region()?;
    let size = b.build_int_const(32, NodeOutputType::U64).unwrap();
...
fn piece_composition_auto_casts_float_input() -> Result<()> {
    let mut b = empty_builder()?;
    let float_val = b.build_float_const(1.0f32.to_bits() as u64, NodeOutputType::F32);
    let int_lo = b.build_int_const(0, NodeOutputType::U32).unwrap();
    ...
    let shift_amt = b.build_int_const(lo_bits, out_ty).unwrap();
