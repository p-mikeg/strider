...
impl BuiltCallingConvention {
    ...
    fn preserves_all_bytes_of(&self, vn: &rsleigh::Vn) -> bool {
        let end = vn.addr_off.saturating_add(u64::from(vn.size));
        let mut covered_to = vn.addr_off;
        while covered_to < end {
            let Some(reach) = self
                .callee_saved_regs
                .iter()
                .filter(|s| s.addr_space == vn.addr_space && s.addr_off <= covered_to)
                .map(|s| s.addr_off.saturating_add(u64::from(s.size)))
    ...
    /// Returns a variant that clobbers nothing: every register callee-saved
    /// and memory unchanged, with no arguments or return value. Models a
    /// transparent hook (`__fentry__` / `mcount`) under this convention's
    /// stack/link-register geometry.
...
const MIPS_O32_BASE: CallingConvention = CallingConvention {
    ...
    callee_saved_regs: &[
        ...
        // loads, and a PIC caller needing `gp` after a call reloads it.  N64,
        // which shares this list, does make `gp` callee-saved.
...
pub(crate) static CC_PRESETS: &[CcPresetRow] = &[
    ...
    CcPresetRow {
        ...
        cc: CallingConvention {
            ...
            // n64 widens the float argument bank over o32's callee-saved
            // $f20-$f23, leaving $f24-$f31 preserved.
            callee_saved_regs: &[
