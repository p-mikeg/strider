...
///
/// Per-analysis, the way a calling convention is: it states what one binary's
/// build of the op does. The tables hold what is true of the ARCHITECTURE.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct CallOtherOverrides(Vec<(String, CallOtherOverride)>);

impl CallOtherOverrides {
    /// # Errors
    /// If a name appears twice. `get` takes the first, so a duplicate would
    /// silently drop the later correction.
    pub fn new(entries: Vec<(String, CallOtherOverride)>) -> Result<Self, String> {
        for (i, (name, _)) in entries.iter().enumerate() {
            if entries[..i].iter().any(|(n, _)| n == name) {
                return Err(format!("duplicate call-other override for {name:?}"));
            }
        }
        Ok(Self(entries))
    }
...
/// Thumb `msr control, Rn` selecting MSP or PSP: `sp` alone re-banks.
const ARM_STACK_SWITCH: CallOtherClass = CallOtherClass::Call(CallOtherAbi {
    implicit_reads: &[],
    implicit_writes: &["sp"],
    clobbers_memory: false,
    no_return: false,
...
///
/// Sleigh's `syscall()` has no pcode output, so this is the whole footprint.
/// The kernel ABI makes r0, r4-r12 and cr0 volatile, so naming only r3 would
/// forward a stale r9-r12 across the call. `XER` names only the SPR the
/// `mfspr` path reads; the bits `adde`/`addc`/`subfe` actually read and write
/// are the separate 1-byte registers at `offset=0x400`, so naming `XER` alone
/// leaves a carry live across the call.
...
static PPC_TABLE: &[(&str, CallOtherClass)] = &[
    ...
    ("trapDoubleWord", MEM_CLOBBER),
    ("trapDoubleWordImmediate", MEM_CLOBBER),
    ("trapWord", MEM_CLOBBER),
    // The `vsx*` prefix below classifies VSX as pure register compute, which
    // holds for every form whose memory access the sla spells as a p-code Load
    // or Store. These nineteen are the exceptions: `vsx.sinc` emits a bare
    // pcodeop with the base register as an operand and no `*[ram]` anywhere, so
    // the access is invisible unless the row names it. Left pure, a `stxvl`
    // does not advance the memory token and a later load of the same address
    // forwards past it.
    ("vsx207_1", MEM_CLOBBER),  // lxsiwax
    ("vsx207_2", MEM_CLOBBER),  // lxsiwzx
    ("vsx207_3", MEM_CLOBBER),  // lxsspx
    ("vsx300_10", MEM_CLOBBER), // lxvll
    ("vsx300_11", MEM_CLOBBER), // lxvwsx
    ("vsx300_15", MEM_CLOBBER), // stxsd
    ("vsx300_16", MEM_CLOBBER), // stxsibx
    ("vsx300_17", MEM_CLOBBER), // stxsihx
    ("vsx300_18", MEM_CLOBBER), // stxssp
    ("vsx300_2", MEM_CLOBBER),  // lxsibzx
    ("vsx300_20", MEM_CLOBBER), // stxvb16x
    ("vsx300_21", MEM_CLOBBER), // stxvh8x
    ("vsx300_22", MEM_CLOBBER), // stxvl
    ("vsx300_23", MEM_CLOBBER), // stxvll
    ("vsx300_3", MEM_CLOBBER),  // lxsihzx
    ("vsx300_4", MEM_CLOBBER),  // lxssp
    ("vsx300_7", MEM_CLOBBER),  // lxvb16x
    ("vsx300_8", MEM_CLOBBER),  // lxvh8x
    ("vsx300_9", MEM_CLOBBER),  // lxvl
...
static ARCH_SPECIFIC_TABLE: &[CallOtherRow] = &[
    ...
    //
    // The cost: with no argument register listed, an `int 0x80` handed a stack
    // address in EBX gives that value no consumer node, so
    // `frame_address_escapes` does not see the escape.  ARM `software_interrupt`
    // and x86-64 `syscall` list theirs and are covered; only
    // `assumptions.escape_analysis` reads that verdict.
    CallOtherRow {
    ...
    // pair (`:5078-5079`, `Dd = SatQ(Dd, esize, unsigned)`), and the SHA-256
    ...
    CallOtherRow {
        ...
        class: ARM_MODE_SWITCH,
    },
    // Declared by `ARMTHUMBinstructions.sinc` alone, so also ARM-32 only.
    CallOtherRow {
        preset_arches: ARM32_ALL,
        op_names: &["setStackMode"],
        class: ARM_STACK_SWITCH,
    },
    ...
    // Linux MIPS o32 SYSCALL: v0 = number, a0..a3 = args, v0 = return value,
    // a3 = error flag, v1 = the second half of a two-value return (`pipe`).
    // Sleigh's `:syscall is ... breakcode { tmp:4=breakcode; syscall(tmp); }`
    // passes only the encoded code field, so the whole register ABI is
    // implicit here.  The caller-saved temporaries are listed as written
    // because the ABI does not preserve them across the kernel entry.
    CallOtherRow {
        preset_arches: MIPS32_BOTH,
        op_names: &["syscall"],
        class: CallOtherClass::Call(CallOtherAbi {
            implicit_reads: &["v0", "a0", "a1", "a2", "a3"],
            implicit_writes: &[
                "v0", "v1", "a3", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9",
            ],
    ...
    // The same op under n64, which passes eight argument registers.  GHIDRA's
    // MIPS spec has no `a4`..`a7` names: those are `t0`..`t3` in `mips.sinc`'s
    // one register list, so the extra four arguments are read from there.
    CallOtherRow {
        preset_arches: MIPS64_BOTH,
        op_names: &["syscall"],
        class: CallOtherClass::Call(CallOtherAbi {
            implicit_reads: &["v0", "a0", "a1", "a2", "a3", "t0", "t1", "t2", "t3"],
            implicit_writes: &[
                "v0", "v1", "a3", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9",
            ],
            clobbers_memory: true,
...
const MIPS32_BOTH: &[crate::ArchPreset] =
    &[crate::ArchPreset::MipsBe32, crate::ArchPreset::MipsLe32];
const MIPS64_BOTH: &[crate::ArchPreset] =
    &[crate::ArchPreset::MipsBe64, crate::ArchPreset::MipsLe64];
...
static ARCH_INDEPENDENT_TABLE: &[(&str, CallOtherClass)] = &[
    // ARM `SETEND` writes CPSR.E, byte-reversing every later DATA access, and
    // the sla says it cannot model that ("we can not change the effective
    // endianness of the language", ARMinstructions.sinc).  A load after one
    // does not read what a store before it wrote, so the memory edge has to
    // break.
    ("setEndianState", MEM_CLOBBER),
    ...
    ("sha256msg1_sha", PURE),
    ("sha256msg2_sha", PURE),
    // SVE / multi-precision: Sleigh's pcode carries the operand registers, so
    // the user-op is pure compute.  The `NEON_` names are the aarch64 prefix
    // family's, not rows here.
    ("MP_INT_ABS", PURE),
    ("SVE_fnmla", PURE),
    ...
    // monitor flag is synthetic, no RAM effect.  CLREX drops the monitor and
    // is emitted as a bare `ClearExclusiveLocal();` with no operands and no
    // output, so it has no footprint at all.
    ("ClearExclusiveLocal", PURE),
    ("ExclusiveAccess", PURE),
    ("hasExclusiveAccess", PURE),
    // ARM PLD / PLDW / PLI / YIELD are architectural no-ops, a prefetch being
    // a cache HINT rather than an observable access, but they stay visible
    // pure markers so patterns can find them.  All three preload forms take
    // the address as a VALUE (`addrmode2` exports `rn + offset`), not a
    // dynamic memory varnode, so none emits a p-code Load.
    ("HintPreloadData", PURE),
    ("HintPreloadDataForWrite", PURE),
    ("HintPreloadInstruction", PURE),
    ("HintYield", PURE),
    ...
    // NEON scalar conversions and polynomial multiply: pure register compute
    // with pcode-explicit operands, companions to the `Vector*` prefix family.
    // The float NEON ops are the `Float*` family, not rows here.
    ("FPToFixed", PURE),
    ("FixedToFP", PURE),
    ...
    ("disableDataAbortInterrupts", PURE),
    ...
    // destination: the CPSR I / F mask bits (`mrs Rd, primask` /
    // `mrs Rd, faultmask`) and the Thumb `msr control, Rn` / `mrs Rd, control`
    // pair between them read the lot.
    ("isCurrentModePrivileged", PURE),
    ("isFIQinterruptsEnabled", PURE),
    ("isIRQinterruptsEnabled", PURE),
    ("isThreadMode", PURE),
    ("isThreadModePrivileged", PURE),
    ("isUsingMainStack", PURE),
    // Setting the privilege bit re-banks nothing; the mode switches that do
    // name `sp` / `lr` and live in `ARCH_SPECIFIC_TABLE`.
    ...
    // fails the lift as an unknown CallOther user-op.
    ...
    // both, a MIPS binary containing SYNC fails the lift as an unknown
    // CallOther user-op.
    ("SYNC", MEM_CLOBBER),
    ("synch", MEM_CLOBBER),
...
fn classify_prefix_family(preset: crate::ArchPreset, name: &str) -> Option<CallOtherClass> {
    ...
    if is_arm32 && (name.starts_with("Vector") || name.starts_with("Float")) {
        return Some(PURE);
    }
    if is_aarch64
    ...
    if is_arm32 {
        // Classified by operation, not direction.  `TLB_Type` is a pure ID
        // read (the real TLB ops are all named `Invalidate_*`); its sibling
        // `Cache_Type` is not, the `Cache` key below sweeping it into
        // MEM_CLOBBER.  Conservative, so sound but imprecise.
...
mod tests {
    ...
    fn ppc_call_others_classify_by_effect() {
        ...
        // The `vsx*` forms whose base-register access the sla leaves out of the
        // p-code must beat the prefix, or a load forwards across the store.
        for n in [
            "vsx300_15",
            "vsx300_20",
            "vsx300_22",
            ...
            assert!(
                mem(n),
                "{n} names memory the p-code does not, so it must clobber"
            );
        }
        ...
        for n in ["trapWord", "trapDoubleWord", "trapDoubleWordImmediate"] {
    ...
    /// The table's stated invariant, enforced over the table itself rather
    /// than a hand-kept name list: a named register resolves only on the arch
    /// whose Sleigh table holds it, so a row naming one builds on that arch
    /// and errors on every other.
    #[test]
    fn arch_independent_call_entries_have_empty_register_channels() {
        for (name, class) in ARCH_INDEPENDENT_TABLE {
            ...
            assert!(
                abi.implicit_reads.is_empty(),
                "arch-independent row {name:?} has non-empty implicit_reads \
                 ({:?}); move it to ARCH_SPECIFIC_TABLE",
                abi.implicit_reads,
            );
            assert!(
                abi.implicit_writes.is_empty(),
                "arch-independent row {name:?} has non-empty implicit_writes \
                 ({:?}); move it to ARCH_SPECIFIC_TABLE",
                abi.implicit_writes,
            );
        }
    ...
    fn arch_independent_entries_resolve_on_every_arch() {
        for (name, class) in ARCH_INDEPENDENT_TABLE {
            for preset in crate::ArchPreset::ALL {
                ...
                assert_eq!(classify(*preset, name), Some(shadow), "{preset:?}/{name}");
            }
        }
    }
    ...
    /// them any PowerPC binary containing a barrier would fail the lift as an
    /// unknown CallOther user-op.
    #[test]
    ...
    /// fail the lift as an unknown CallOther user-op.
    #[test]
    ...
    fn prefix_families_are_arch_scoped() {
        ...
        // `Vector*` and `Float*` are ARM-32's NEON spelling; AArch64 uses
        // `NEON_*` and declares neither.
        assert!(is_pure(classify(Arm, "VectorMultiply")));
        assert!(is_pure(classify(Arm, "FloatVectorMult")));
        assert_eq!(classify(Aarch64, "VectorMultiply"), None);
