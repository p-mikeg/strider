...
```python
prog.symbol("array_sum")     # a Symbol: name, address, size, end, region
...
for m in function.find_all(
    [load(addr=var(base)), load(addr=int_add(var(base), int_const(off)))],
    ignore_casts=True,
):
    print("second field at offset", m.uint(off))
```
...
The `find` side is a `strider.pattern` pattern; the `replace` side is a
`strider.template` template, which covers the ops that can be built.
...
the hub cap, and whether to render pretty. They start at the same defaults
`neighborhood_dot` uses, and `reset` puts them back.
