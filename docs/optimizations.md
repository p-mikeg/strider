...
Before you query a function, Strider optimizes its IR, for regularity rather
than for speed. Optimization folds away clutter and rewrites equivalent code
...
each one. Register-passed arguments are recorded at lift time, so they are
already in place before this pass runs.
...
any site whose answer shrank between rounds. A converged CFG is never silently
incomplete. `analyze` raises only when the loop oscillates without settling; a
chain deeper than the iteration cap comes back through `unresolved` instead.
...
own, which the loop then grows from, and
`LifterOptions(resolve_indirect_branches=False)` turns the classifier off and
leaves every site for you to answer.

## Using a different pipeline

Build a custom set of passes with the `strider.opt` builders and pass it through
`LifterOptions(pipeline=...)`. `IndirectBranchClassify` runs whatever you list:
it is appended by `analyze` and `strider.opt` does not expose it, so
`resolve_indirect_branches=False` is how you leave it out.
