...
- **A dedicated `strider-ir-test-support` crate** when multiple downstream crates need them as dev-deps.
...
### Task D.3: Extract `TestGraph` to a dedicated `strider-ir-test-support` crate
...
**Files:**
- Create: `crates/strider-ir-test-support/Cargo.toml`
- Create: `crates/strider-ir-test-support/src/lib.rs`
- Move: `crates/strider-ir/src/test_helpers.rs` → `crates/strider-ir-test-support/src/lib.rs` (renamed module)
- Modify: `crates/strider-ir/Cargo.toml` — drop the `test-utils` feature.
- Modify: Every downstream crate's `Cargo.toml` — replace `strider-ir = { workspace = true, features = ["test-utils"] }` in dev-deps with `strider-ir-test-support = { workspace = true }`.

- [ ] **Step 1: Create the new crate**

```bash
mkdir -p crates/strider-ir-test-support/src
```

`crates/strider-ir-test-support/Cargo.toml`:
```toml
[package]
name = "strider-ir-test-support"
...
`crates/strider-ir-test-support/src/lib.rs`:
```rust
...
```toml
strider-ir-test-support = { path = "crates/strider-ir-test-support" }
```
...
# After:
strider-ir = { workspace = true }
strider-ir-test-support = { workspace = true }
```
...
```bash
git commit -m "feat(strider-ir-test-support): extract TestGraph to a dedicated crate

strider-ir's src/ no longer contains test code. The 'test-utils'
feature is gone. Downstream tests dev-dep on strider-ir-test-support
...
```bash
for c in strider-ir strider-ir-test-support strider-lift strider-analyze strider-py strider target reader strider-pattern-macros; do
    echo "=== $c ==="
...
| D.3 | TestGraph moves to dedicated `strider-ir-test-support` crate | Multiple downstream crates need it; tests/common/ in strider-ir isn't reachable cross-crate. |
