...
> **Purpose.** This file is a self-contained prompt the user can paste back into Claude later to drive a fresh, independent code review of the strider workspace.  It assumes prior rounds 7–12 have already landed — see `reviews/round7-*.md` through `reviews/round12-*.md` — and that the codebase has continued to evolve since.  This round must rederive its findings from the *current* code, not from prior reviews.
...
I want you to do another round of deep code review on the strider workspace at `/mnt/c/Users/mikeg/Documents/strider`.  This is round **13**.

**It is genuinely OK to report "no findings" for an axis or category.**  If a class of issues has been comprehensively cleaned up by prior rounds and a thorough re-derivation from the current code surfaces nothing new, say so explicitly — "verified consistent — no finding" with a one-line evidence pointer is a complete and valuable answer.  Don't manufacture findings to fill quota.  The signal that an area is now clean is itself useful; what we want is honest re-derivation, not a guaranteed minimum count.  Conversely, when something IS wrong, the strict trust model still applies — cite `file:line`, verify against rsleigh / ABI specs, and provide a concrete fix.

## Trust model — strict

- **Do NOT read `reviews/round7-*.md` through `reviews/round12-*.md` or any earlier-round audit as authoritative input.**  You may at most note that an item was flagged before and re-derive the finding from scratch.  The previous reviews are stale relative to the current branch state — the code has evolved since they were written, sometimes substantially.  Each finding in your final summary must be derived independently from the current source.
...
3. **Lifted-IR-vs-assembly correctness.**  Pick a real binary on each supported arch and verify the IR's behaviour matches what the binary *actually does* at runtime.  This is the deepest check — it catches bugs neither code-vs-code nor IR-vs-pcode can catch (an architectural pcode bug in rsleigh, a missing canonicalisation, a clobber-set mismatch).
   ...
   - **Memory chain after a call.**  `LoadReadOnly` and `LoadForward` must stop forwarding across normal calls AND must forward across all-preserving (zero-side-effect) hooks like `__fentry__` / `mcount`.  Pick a real Linux-kernel binary with `__fentry__` and verify that rodata loads after the hook still fold to constants.
...
The simplifications report should land at `reviews/round13-simplifications.md` with sections matching the toolkit above:
...
This review is **exhaustive**, not sampled.  Every `.rs` file under `crates/*/src/`, every `.rs` file under `crates/*/tests/` and `crates/*/benches/`, every `.py` file under `crates/strider-py/tests/python/`, every `Cargo.toml`, every `*.md` file under `crates/*/README.md` + the root README + CLAUDE.md, must be **read in full** by at least one subagent during the rounds below.  (Skills under `crates/strider/.claude/skills/*/SKILL.md` are explicitly out of scope this round — round 11 last audited them; the next pass to revisit them is whichever future round wants to.)

Concretely:

- **Inventory first.**  The Round 0 orientation step must produce `reviews/round13-coverage-manifest.md` listing every file in scope (use `find crates -name '*.rs' -o -name '*.toml'` + `find crates/strider-py/tests -name '*.py'` + the doc set).  Every file in that manifest must be ticked off as "inspected by subagent X" by the time Round 7 (final consolidation) runs.
...
3. Be told explicitly: do NOT read `reviews/round7-*.md` through `reviews/round12-*.md` or any earlier-round output.
4. Produce a single Markdown report at `reviews/round13-<topic>.md` with HIGH/MED/LOW findings, each with `file:line` and a concrete fix.
...
3. **Correctness — lifted IR vs real assembly** *(emphasis A axis 3; deepest correctness check)*.  Per arch (x86, x86_64, ARM, ARM-BE, ARM-Thumb, AArch64, AArch64-BE, MIPS32-LE/BE, MIPS64-LE/BE, PPC32-LE/BE, PPC64-LE/BE), pick one representative function from `fixtures/out/<arch>/` and verify:
   ...
   - **Memory chain after a call.**  Calls advance the memory edge by default (modulo `no_memory_clobber`).  Verify that `LoadReadOnly` and `LoadForward` correctly stop forwarding across normal calls AND correctly forward across all-preserving calls.  Pick a real Linux-kernel binary with `__fentry__` instrumentation and verify the rodata loads after the hook still fold to constants.
...
8. **Multiple rounds of correctness audit, rotating focus.**
   ...
   Each pass produces its own subagent report (`reviews/round13-correctness-types.md`, `round13-correctness-invariants.md`, `round13-correctness-borrowing.md`, `round13-correctness-edge-cases.md`, `round13-correctness-cross-arch.md`).
...
13. *(Reserved — skill audits are out of scope this round; round 11 last covered them.)*
...
14. **Scale + Performance at thousands-of-nodes scale.**  Verify the codebase is *optimised* for ~10k–100k IR nodes, not merely correct.  Per hot path, derive the asymptotic complexity from the code (don't trust comments), then check:
    ...
    - **`Match::find_joined` cross-product blowup.**  For shared-capture queries with M patterns each matching N nodes, the worst case is O(N^M).  Verify whether better pruning (sort by pattern selectivity, prune at first disagreement instead of full-scan) is feasible without breaking the binding-agreement contract.
...
### Round 0 — orient
Read CLAUDE.md, the workspace `Cargo.toml`, the per-crate `Cargo.toml` files, every per-crate README.  Build a mental model of what's in each crate.  Run the four baseline checks above.  Produce `reviews/round13-coverage-manifest.md` listing every file in scope.
...
- **3A — trust-only-the-code verification.**  Sample ≥ 25 specific claims from CLAUDE.md and per-crate READMEs.  For each, find the code and confirm or refute purely from code shape.
...
`feature-dev:code-architect`.  Consume Round 1 outputs.  Emit `reviews/round13-test-plan.md` listing each missing test with: scope, file path, harness/fixture, expected assertions, estimated effort.  Use TDD discipline — failing test FIRST.
...
- **pattern `find_joined` shared-capture-disagreement filtering.**
...
`pr-review-toolkit:code-simplifier`.  Consume Rounds 1–3.  Emit `reviews/round13-simplifications.md` per emphasis B's seven-category toolkit:
...
1. `reviews/round13-summary.md` — executive summary with prioritised fix backlog.
2. `reviews/round13-claudemd-diff.md` — concrete CLAUDE.md edits.
3. `reviews/round13-readme-diffs.md` — concrete per-crate README edits.

## Acceptance criteria

- [ ] Every numbered ask (1–18) has a corresponding section in `reviews/round13-summary.md` with concrete actions.
- [ ] Emphasis A produces three reports: `reviews/round13-correctness-self-vs-self.md`, `reviews/round13-correctness-ir-vs-pcode.md`, `reviews/round13-correctness-ir-vs-assembly.md`.  Each finding cites code locations + ABI / opcode references + (for axis 3) the real binary's `objdump` trace.
- [ ] Emphasis B produces `reviews/round13-simplifications.md` with 50–80 entries grouped into the seven-category toolkit.  Header includes the per-category total LOC delta and the projected post-implementation workspace LOC reduction.
- [ ] Ask 8 produces five separate reports — `round13-correctness-types.md`, `round13-correctness-invariants.md`, `round13-correctness-borrowing.md`, `round13-correctness-edge-cases.md`, `round13-correctness-cross-arch.md` — each from a fresh subagent that did not read the others.
- [ ] Round 13 summary lists every HIGH-severity finding with `file:line` + a proposed fix + (for emphasis A findings) the regression test scaffolding.
...
- [ ] Ask 3 produces `reviews/round13-correctness-ir-vs-assembly.md` documenting per-arch ABI verification against real binaries.  Every finding includes the lifted-from-real-code regression test that pins it.
...
- [ ] No source code is edited during the review.  The output is the set of `reviews/round13-*.md` reports.  Implementation is a follow-up task that the user will explicitly approve.
- [ ] `reviews/round13-coverage-manifest.md` exists and shows every `.rs` / `.py` / `Cargo.toml` file under `crates/` ticked off as "inspected fully" by at least one subagent.

## Out of scope

- **Editing source code.**  Only documentation under `reviews/round13-*.md` is allowed.
...
- Optimizer: every `crates/opt/src/<pass>/mod.rs`, plus `pipeline.rs`, `sp_expr.rs`, `worklist.rs`, `test_support.rs`, `indirect_branch_resolve/{mod,jump_table,stack_array,classify,inplace}.rs`, `load_forward/mod.rs`, `function_args/mod.rs`, `flag_cmp_canonicalize/mod.rs`, `if_cond_inversion/mod.rs`, `known_bits/mod.rs`, `stack_store/{mod,detect}.rs`
...
The review is itself a research effort — the "verification" is the quality of the final summary, not a build.  The acceptance criteria above are the bar.  At the end, leave a short note in `reviews/round13-summary.md` describing:
...
- The prompt explicitly forbids reading `reviews/round7-*.md` through `reviews/round12-*.md` so the next round derives findings independently.  The previous rounds' outputs stay on disk as historical context.
...
- After the review lands, you'll have the `reviews/round13-*.md` set — at that point a follow-up prompt of the form "land everything not refuted from round 13" runs the same play we ran for rounds 7–12.
