...
1. **Correctness — code vs code self-consistency** *(emphasis A axis 1)*.  For every operation implemented in 2+ sites, verify the implementations agree on every input.  Sites to start from:
   - SP decomposition: `decompose_sp` AND `match_stack_array_shape` AND `CallStackArgCollect` AND `function_args::mem_chain_is_dirty`.
   - CallOther dispatch: `cfg::region_builder` (terminator) AND `strider::IrStrider::handle_call_other` (IR emission).
   - Varnode aliasing: `pcode_lift::vn_io::read_vn` AND `write_vn` (must be inverses for any matched pair).
   - Commutativity: `pattern::matcher::commutativity` table AND the build-RHS path AND every per-op family in `pat::ctor::int` / `pat::ctor::bool_` / `pat::ctor::float`.
   - Lift-time canonicalisations: `pcode_lift::value::*` lowering AND the pattern crate's lowered aliases (`sub`, `int_le`, `int_sle`, `float_sub`, `float_ne`, `float_le`).
   - Asm-fingerprint contract: every opt pass that creates a node must absorb contributors via `extend_asm_fingerprint_from` (round-8 fixed `IfCondInversion` + `StackLoadForward` BE narrow path; verify the contract holds in every other pass too).
   - Validator Layer A/B/C reachability scoping: round 8 fixed `check_layer_c_function_arg_uniqueness`; verify every other Layer-C check follows the same scoping rule.

   Edge cases to specifically verify across paired sites: register aliasing for every width in `vn_mask` (1, 2, 4, 8, 10, 16, 32, 64); bounded-lift `is_addr_tail_call` half-open semantics; indirect branches for every shape the resolver claims to handle; memory-edge clobbering on `Call` / `Store` / `CallOther`; phi shapes (VarPhi / MemPhi / StackStorePhi / ValuePhi arity rules); NaN ordering on float compares; sign / zero extension; all eight lift-time canonicalisations (`IntSub`, `IntLessEqual`, `IntSlessEqual`, `IntNotEqual`, `FloatSub`, `FloatNotEqual`, `FloatLessEqual`, `FLOAT_NAN`); CallOther ABI classifications.
...
3. **Correctness — lifted IR vs real assembly** *(emphasis A axis 3; deepest correctness check)*.  Per arch (x86, x86_64, ARM, ARM-BE, ARM-Thumb, AArch64, AArch64-BE, MIPS32-LE/BE, MIPS64-LE/BE, PPC32-LE/BE, PPC64-LE/BE), pick one representative function from `fixtures/out/<arch>/` and verify:
   ...
   - **CallOther implicit reads / writes.**  For each entry in `target::call_other_abi::classify`, fetch the Intel SDM / ARM ARM / MIPS Reference / PowerPC ISA reference for the underlying instruction and verify `implicit_reads` covers every register the instruction reads and `implicit_writes` covers every register it writes.  **Sample at least 20 entries** — `cpuid`, `rdtsc`, `rdtscp`, `rdmsr`, `wrmsr`, `swapgs`, `wrgsbase`, `wrfsbase`, `mfence`, `sfence`, `lfence`, `cmpxchg16b`, `xsetbv`, `xgetbv`, `monitor`, `mwait`, `int 0x80`, `syscall`, `sysret`, `sysenter` — plus the round-8-added fence ops + the per-arch `swi` / SMCCC entries.
   - **Indirect-branch resolution against real targets.**  For each shape the resolver claims to handle (link-register return, jump-table, stack-array dispatch, tail-call, the round-8-follow-up `Truncate(IntConst)` / `Extend(IntConst)` arms), find a real binary exhibiting that shape and verify the resolved target set matches the symbol-table truth (`nm` / `addr2line`) for **at least three call sites per shape**.  The 7 ignored tests in `crates/strider/tests/indirect_branch.rs` document specific shapes (`aarch64-be Or(SP,K) + Truncate-wrapped labels`, `mips64 PIC GOT-indirect`, `ppc32/64 uncharacterised`); fact-check the documented fix paths against the actual lifter output.
   - **Lift-time canonicalisations are bit-exact.**  Pcode lowering for all eight canonicalisations — `IntSub` / `IntLessEqual` / `IntSlessEqual` / `IntNotEqual` / `FloatSub` / `FloatNotEqual` / `FloatLessEqual` / `FLOAT_NAN` — was verified once.  This round re-verifies on **at least one real lifted instruction per canonicalisation**, comparing the IR against a hand-derived expected pcode trace from rsleigh.  Special edge cases: `IntSub` on `(a, INT_MIN)` (modular negation), `FloatLessEqual` on NaN inputs, `FLOAT_NAN(x)` on signaling NaN.
...
14. **Scale + Performance at thousands-of-nodes scale.**  Verify the codebase is *optimised* for ~10k–100k IR nodes, not merely correct.  Per hot path, derive the asymptotic complexity from the code (don't trust comments), then check:
    - **Recursion-induced stack-overflow risk** in any function that walks a memory or control chain without an explicit depth bound and without an iterative form.  Round 8 converted `decompose_sp` to iterative; verify no other recursive walker remains, especially in `MemPhi` predecessor walks or `ControlState` predecessor walks.
    ...
    - **Hash sets / maps over `NodeId` / `NodeOutputId` / `NodeInputId`.**  Replace with `entity_utils::DenseEntitySet` / `cranelift_entity::SecondaryMap` where the entity is dense — the bit-vector / array shape skips the FxHash entirely and gets cache-local indexing.  Round 8 covered `WorkSet::queued`, `detach_unreachable_nodes::reachable`, `KnownBits::analyze` return type; this round looks for the missed sites.  No remaining `FxHashSet<NodeId>` / `FxHashMap<NodeOutputId, _>` should exist in hot paths.
    - **Repeated full-graph scans inside fixed-point loops.**  Anything in `OptimizerPipeline::run`'s loop body or `strider::orchestrator::step` that walks `graph.all_node_ids()` is O(N) per iteration; in the orchestrator's worst case (`2 * pending_at_iter_0 + 4` iterations) this is multiplicative.  Identify scans that can be cached / amortised across iterations or replaced with a worklist.  Round 8 fixed `apply_in_place_edits` to use `preorder()` instead of `all_node_ids()`; check for siblings.
    ...
    - **Memory residency.**  After `orchestrator::run` finishes, what's still alive?  Side-tables (`asm_fingerprints`, `wide_consts`, `call_other_names`, `stack_phi_offsets`, `call_clobbered_override`) keyed on detached-zombie `NodeId`s are wasted bytes.  Verify `compact()` is called when expected and that all side-tables are GC'd by it (the `gc_wide_consts` path landed in round 7; verify equivalent paths exist for the others).
...
- **ARM/AArch64/MIPS end-to-end ELF fixture coverage** for the seven currently-`#[ignore]`'d shapes in `crates/strider/tests/indirect_branch.rs`.
...
- **Iterative-form regression for any recursive memory-chain walk**: chain of 1k+ stores must not stack-overflow (round 8 added `decompose_sp` 5000-node test; verify equivalent coverage for every other walker).
- **`crates/strider/src/test_utils.rs` per-arch wrappers** — one smoke test per `strider_for_arch` variant to keep them in sync with their corresponding CC presets.
...
- **Cross-arch CallOther round-trip via `strider::run`** — at minimum one ARM `swi`, one AArch64 SMCCC, one x86_64 syscall lifted end-to-end (round 8 noted the orchestrator preset bug; this test pins the fix).
