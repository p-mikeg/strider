...
Every user-facing Python API, in depth, with a runnable example of each. For the
practical, task-first walkthrough instead, read the
...
groups rather than enumerates). Everything here runs against the committed
fixture ELFs in `fixtures/out/x86/`.
...
strider.StriderError      # the one exception type every call raises
strider.__version__
```
...
# apply_relocations=True (default) patches relocations in place.
```
...
lift = strider.lift.lifter(sleigh.SleighArch.arm(), Firmware(open("fw.bin","rb").read()))
```
...
opts = strider.lift.LifterOptions(
    ...
    alias_mode="strict",               # or "stack_global_disjoint"
...
strider cannot check, so a wrong one can make the answer wrong; everything
outside it is sound at every setting.
...
strider.sleigh.CallOtherAbi.custom(
    sleigh,                       # resolves the register names, here and now
...
n.kind()                       # "IntBinaryOp(Add)", "Load(Ram)", "Region", ...
...
keeps the order you wrote. To pin the order on a commutative one use the
`int_binary` / `float_binary` / `bool_binary` builder (the `int_add` / `int_mul` / ...
sugar returns a finished `Pat`, whose `.ordered()` raises and points you here):
...
SysV `.arg(6)` is the first float argument. The integer positions come from the
convention's register list, while only the float registers the analyzed function
names are collected, so which `xmm` a float index lands on varies by call site.
The incoming-argument patterns index the two classes separately instead.
...
| `InputPat` | `.input(i, p)`, `.any_input(p)` | every builder but `entry()` (`Entry` has no inputs) |
...
| `OutputPat` | `.output(slot)`, `.any_output()` | every builder but the sinks `ret`, `indirect_branch`, `unreachable` (no outputs) |
...
function.find_all(pat, ignore_casts=True)    # see through width casts (usually on)
...
for hit in function.find_all(p.load(addr=p.int_add(p.anything(), p.int_const(off))),
                             ignore_casts=True):
    print("field at offset", hit[off].uint)        # 4, then 8
```
...
register names), with
**reset** to go back. Every control starts at the renderer's own default, the
one `neighborhood_dot` uses. Off the main thread you must pair it with
...
Every failure is a `strider.StriderError`. Match readers additionally raise it
...
`analyze` raises it when indirect-branch resolution oscillates without settling.
An unresolvable site, a site whose answer shrank between rounds, and a target
chain deeper than the iteration cap all come back in `unresolved` instead.
...
`os.environ` and takes effect on the next error. `RUST_BACKTRACE=0` in the
