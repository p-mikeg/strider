...
**Resolution (2026-05-08): SKIP.** Investigation confirmed the sparse-init concern is structurally non-exploitable through the public API. The `VarId` set is **frozen** in `FunctionBuilder::new_raw` (`builder/mod.rs:331-336` populates `variables: PrimaryMap` once; nothing else in the crate ever calls `variables.push` later). Both region constructors — `set_entry_region` (`builder/vars.rs:75-83`) and `create_region` (`builder/vars.rs:113-120`) — iterate the full `self.variables.keys()` and produce a `NodeOutputId` per `VarId` before calling `create_region_helper`, which seeds `Region.variables` via `initial_variables.clone()` (`region.rs:176`). So every region's `variables` map is fully populated for every tracked `VarId` at the moment it becomes reachable. Switching to `Option<NodeOutputId>` would push a "fall back to entry-time value" obligation into every read path (`read_variable_from_id`, `link_region_variables`, `region_exit_variables`) — currently the cloned-from invariant makes this trivial — without delivering any pass that actually wants to ask "was this variable written in this region?" (no caller asks today; the question can be answered by comparing `variables[v] == initial_variables[v]` if it ever arises). The proposed change ripples through the read/write/link/exit-iteration paths, replacing one structurally-trivial invariant with code-enforced fallback logic, for no concrete simplification downstream. The remaining `NodeOutputId(0)` "silent wrong edge" failure mode is not reachable through the public API. Closed without code change.
